...
## -----------------------------------------------------------------------------
tree <- rpart(formula = (log(mean_chl + 1)) ~ Elevation_m +  Watershed_area_to_lake_area_ratio +
...
#R2 􏰁 1 – relative error
#I think 21 points is just too small for these trees?
#using just the pruned variables - these are not good models
lm.chla<-lm((log(mean_chl + 1)) ~ Elevation_m * solar_jas * forest,
            data = reg_tree_data)
lm.chla
summary(lm.chla)
