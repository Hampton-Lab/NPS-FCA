...
# include/exclude statistical interactions?
#interact<-1
interact<-1
oneiceswe<-1
...
bigjoin.stats$value[which(bigjoin.stats$variable=="DO_top2m" & bigjoin.stats$value<0)]<-1

######################################
# count site years

bigjoin.stats %>% group_by(variable) %>% 
  dplyr::summarize(siteyearcount=length(value)) %>% as.data.frame()
...
sitestable_supp<-sitestable %>% select(-SWE,-iceout_doy,-ice_free_days,
                                       -bst,-btg,-grd,-grw,-qzm) %>% unique() %>% filter(Park!="NA")

write.csv(sitestable_supp,"../figures/analysispowers/sitestable.csv")
...
SWE_days_since_iceout_scatterplot<-ggplot(data=dataplot,aes(y=days_since_iceout,x=SWE_May_snotel,color=park_code,label=short_code))+
  ...
  theme(strip.text.x=element_text(),axis.title.x = element_text(vjust=-0.5))+#facet_wrap(~park_siteshort,nrow=3)+ 
  scale_colour_viridis_d(end = 0.85)
lm<-lm(days_since_iceout~SWE_May_snotel,data=dataplot)
summary(lm)


png(filename = "../figures/analysispowers/iceandsnow_multi.png",
  width = 5, height = 12, units = "in",res=300)
grid.arrange(iceout_SWE_scatterplot,iceoutdoy_SWE_scatterplot,
  SWE_days_since_iceout_scatterplot)
dev.off()

png(filename = "../figures/analysispowers/iceandsnow_multi_welev.png",
...
meanval_SWEmidlohi<-data_forstats %>% group_by(variable=var_abbrev2,snowyr) %>% #snowyrmid_snotel,snowyrlo_snotel,snowyrhi_snotel) %>%
...
meanval_SWEoverall<-data_forstats %>% mutate(snowyr="overall")  %>% group_by(variable=var_abbrev2,snowyr) %>% #snowyrmid_snotel,snowyrlo_snotel,snowyrhi_snotel) %>%
  dplyr::summarize(mean=mean(value, na.rm=TRUE),
                   median=median(value, na.rm=TRUE),
                   min=min(value,na.rm=TRUE),
                   q25th=quantile(value,probs=c(.25),na.rm=TRUE),
...
meanval_SWEmidlohi_iceout<-data_forstats %>%  mutate(variable="ice_out_doy") %>% group_by(variable,snowyr) %>% #snowyrmid_snotel,snowyrlo_snotel,snowyrhi_snotel) %>%
  dplyr::summarize(mean=mean(SWE_May_snotel, na.rm=TRUE),
                   median=median(value, na.rm=TRUE),
                   min=min(SWE_May_snotel,na.rm=TRUE),
                   q25th=quantile(SWE_May_snotel,probs=c(.25),na.rm=TRUE),
                   q75th=quantile(SWE_May_snotel,probs=c(.75),na.rm=TRUE),
                   max=max(SWE_May_snotel,na.rm=TRUE)) %>% as.data.frame()
meanval_SWEmidlohi_iceout_overall<-data_forstats %>% mutate(variable="ice_out_doy",snowyr="overall")  %>% group_by(variable,snowyr) %>% #snowyrmid_snotel,snowyrlo_snotel,snowyrhi_snotel) %>%
  dplyr::summarize(mean=mean(SWE_May_snotel, na.rm=TRUE),
                   median=median(value, na.rm=TRUE),
                   min=min(SWE_May_snotel,na.rm=TRUE),
                   q25th=quantile(SWE_May_snotel,probs=c(.25),na.rm=TRUE),
...
meanval_SWEmidlohi<-rbind(meanval_SWEmidlohi,meanval_SWEoverall,meanval_SWEmidlohi_swe,meanval_SWEmidlohi_swe_overall,
                          meanval_SWEmidlohi_iceout,meanval_SWEmidlohi_iceout_overall) %>% 
  arrange(variable,snowyr) %>% mutate_if(is.numeric, signif, 3)
...
#â„¹ The author of the class should implement vctrs methods.
#â„¹ See <https://vctrs.r-lib.org/reference/faq-error-incompatible-attributes.html>.
...
m_0covar_allvar_out<-m_0covar_allvar_out %>% mutate(b0=paste(val.int," Â± ",se.int,sep=""),
                                                    Mlo=paste(val.lo," Â± ",se.lo,sig.lo,sep=""), Mhi=paste(val.hi," Â± ",se.hi,sig.hi,sep="")) %>% 
...
checktiedyears[which(checktiedyears$minyr!=checktiedyears$maxyr),]
...
crosscorr_hiy$Secchi[is.na(crosscorr_hiy$Secchi)]<-mean(crosscorr_hiy$Secchi,na.rm=TRUE)
crosscorr_loy$Secchi[is.na(crosscorr_loy$Secchi)]<-mean(crosscorr_loy$Secchi,na.rm=TRUE)
...
doplots_set<-0
usenlme<-1
source("models_hypoth_v4.R")

models_df0<-models_df
models_df<-models_df %>% unique()


# some model interactions are beyond the scope- drop these interactions
...
models_df_checks$hasdayssinceice<-0
models_df_checks$hasdayssinceice<-grepl("days_since_iceout",models_df_checks$modname) %>% as.numeric()

which_drops<-which((models_df_checks$hasiceoutd+models_df_checks$hasSWE)==2 | 
                     (models_df_checks$hasdayssinceice+models_df_checks$hasSWE)==2 | 
                     (models_df_checks$hasiceoutd+models_df_checks$hasdayssinceice)==2)

which_drop<-which_drops
...
models_df<-models_df %>% select(-hasiceoutd,-hasSWE,-hasdayssinceice)


# include or exclude interactions
which_interact<-c(grep("\\*",models_df$modname),grep("\\^",models_df$modname)) %>% unique()
if(interact==0 & length(which_interact)>0){models_df<-models_df[-which_interact,]}
...
if(interact==1){
  ...
  two<-one[-grep("ice_out_doy|SWE|days_since_iceout",one$term,ignore.case=TRUE),]
  modnames_drop<-two$modname %>% unique()
...
models_top_signif_df<-models_top_df %>% filter(term!= "(Intercept)" & p.value<=0.99) %>% select(varname,modname,modname2) %>% unique()
...
models_df_onevarwrandom<-models_df %>% filter(nfix==2)

models_top10_df<-models_df %>% filter(AICcdiff<=10) %>% arrange(varname,AICcdiff)
models_top10_df<-models_top10_df %>% select(varname,modelform=modname2,k=nparam,AICc,AICcdiff,R2m,R2c,
  term,estimate,std.error,p.value)
#models_top10_df$model<-""
models_top10_df$modelform[which(models_top10_df$modelform=="vari1")]<-"1"
models_top10_df$modelform[which(models_top10_df$modelform=="vari1+vari2")]<-"2"
models_top10_df$modelform[which(models_top10_df$modelform=="vari1*vari2")]<-"2x"
models_top10_df$modelform[which(models_top10_df$modelform=="vari1+vari2+vari3")]<-"3"
models_top10_df$modelform[which(models_top10_df$modelform=="(vari1+vari2+vari3)^2")]<-"3x"


write.csv(models_df,file=paste(fileout,".csv",sep=""),row.names=FALSE)
write.csv(models_df_onevarwrandom,file=paste(fileout,"_onevarwrandom.csv",sep=""),row.names=FALSE)
write.csv(models_top10_df,file=paste(fileout,"_top10AIC.csv",sep=""),row.names=FALSE)


models_df_swe_iceout_doy<-models_df[grep("SWE||iceout_doy", models_df$modname),]
models_df_swe_iceout_doy<-models_df_swe_iceout_doy %>% relocate(AICcdiff,modname, .after=term)

swe_iceout_doy_compare<-models_df_swe_iceout_doy %>% filter(term %in% c("SWE","iceout_doy")) %>% select(varname,term,AICcdiff,modname) %>% group_by(varname,term) %>% 
  dplyr::summarize(AICcdiff2=min(AICcdiff),modname[which(AICcdiff2==AICcdiff)]) %>% arrange(varname,AICcdiff2)

swe_iceout_doy_compare_simple<-models_df_swe_iceout_doy %>% filter(term %in% c("SWE","iceout_doy"), modname2=="vari1") %>% select(varname,term,AICcdiff,modname) %>% group_by(varname,term) %>% 
  dplyr::summarize(AICcdiff2=min(AICcdiff),modname[which(AICcdiff2==AICcdiff)]) %>% arrange(varname,AICcdiff2)


write.csv(swe_iceout_doy_compare,file="../figures/analysispowers/comparemodels_allhypoth_swe_iceoutdoy.csv")
write.csv(swe_iceout_doy_compare_simple,file="../figures/analysispowers/comparemodels_simplest_swe_iceoutdoy.csv")
write.csv(models_df_swe_iceout_doy,file="../figures/analysispowers/comparemodels_detailed_swe_iceoutdoy.csv")

models_top10_df %>% filter(AICcdiff>2 & AICcdiff<4) %>% 
  select(varname,modelform,k,R2m) %>% unique() %>% 
  group_by(varname,modelform,k) %>%
  dplyr::summarize(countmodels=length(R2m)) %>% as.data.frame()

countmodels_top10<-models_top10_df %>% 
  select(varname,modelform,k,R2m) %>% unique() %>% 
  group_by(varname) %>%
  dplyr::summarize(countmodelstop10=length(R2m)) %>% as.data.frame()

countmodels_top2<-models_top10_df  %>% filter(AICcdiff<2) %>%
  select(varname,modelform,k,R2m) %>% unique() %>% 
  group_by(varname) %>%
  dplyr::summarize(countmodelstop2=length(R2m)) %>% as.data.frame()

countmodels_top2to4<-models_top10_df  %>% filter(AICcdiff>=2 & AICcdiff<4)  %>%
  select(varname,modelform,k,R2m) %>% unique() %>% 
  group_by(varname) %>%
  dplyr::summarize(countmodels2to4=length(R2m)) %>% as.data.frame()

countmodels_top2to6<-models_top10_df  %>% filter(AICcdiff>=2 & AICcdiff<6)  %>%
  select(varname,modelform,k,R2m) %>% unique() %>% 
  group_by(varname) %>%
  dplyr::summarize(countmodels2to6=length(R2m)) %>% as.data.frame()

countmodels_top2to10<-models_top10_df  %>% filter(AICcdiff>=2 & AICcdiff<10)  %>%
  select(varname,modelform,k,R2m) %>% unique() %>% 
  group_by(varname) %>%
  dplyr::summarize(countmodels2to10=length(R2m)) %>% as.data.frame()

merge1<-merge(countmodels_top10,countmodels_top2,
  by=c("varname"),all=TRUE)
merge2<-merge(merge1,countmodels_top2to4,
  by=c("varname"),all=TRUE)
merge3<-merge(merge2,countmodels_top2to6,
  by=c("varname"),all=TRUE)
merge4<-merge(merge3,countmodels_top2to10,
  by=c("varname"),all=TRUE)
countmodels_top_merged<-merge4

write.csv(countmodels_top_merged,"../figures/analysispowers/countmodels_top_merged.csv")
...
models_top_signif_df<-models_df %>% filter(term!= "(Intercept)" & p.value<=0.99) %>% dplyr::select(varname,modname,modname2) %>% unique()
...
covartbl<-
  ...
  rename(p=p.value) %>%
  select(varname,term,AICc,R2m,R2c,coef,p,nparam,modname,modname2,modname3,var0,var1,var2,var3,varint1,varint2,varint3,estimate) %>%
  pivot_wider(names_from=c(var0,var1,var2,var3,varint1,varint2,varint3),values_from=c(term,coef,p,estimate)) %>%
  as.data.frame()
...
covartbl$coef4sign[which(substr(covartbl$estimate4,1,1)>=0)]<-"+"
covartbl$coef4sign[which(substr(covartbl$estimate4,1,1)=="-")]<-"-"
covartbl$coef5sign[which(substr(covartbl$estimate5,1,1)>=0)]<-"+"
covartbl$coef5sign[which(substr(covartbl$estimate5,1,1)=="-")]<-"-"
covartbl$coef6sign[which(substr(covartbl$estimate6,1,1)>=0)]<-"+"
covartbl$coef6sign[which(substr(covartbl$estimate6,1,1)=="-")]<-"-"

#covartbl$coef4<-gsub("",covartbl$coef4,NA)
#covartbl$coef5<-gsub("",covartbl$coef5,NA)
#covartbl$coef6<-gsub("",covartbl$coef6,NA)
if(unique(covartbl$coef4)==""){
  covartbl$coef4<-""
  covartbl$p4<-""
  covartbl$term4<-""
}
if(unique(covartbl$coef5)==""){
  covartbl$coef5<-""
  covartbl$p5<-""
  covartbl$term5<-""
}
if(unique(covartbl$coef6)==""){
  covartbl$coef6<-""
  covartbl$p6<-""
  covartbl$term6<-""
}
...
covartbl$model<-""
covartbl$model[which(covartbl$modname2=="vari1")]<-"1"
covartbl$model[which(covartbl$modname2=="vari1+vari2")]<-"2"
covartbl$model[which(covartbl$modname2=="vari1*vari2")]<-"2x"
covartbl$model[which(covartbl$modname2=="vari1+vari2+vari3")]<-"3"
covartbl$model[which(covartbl$modname2=="(vari1+vari2+vari3)^2")]<-"3x"
...
#covartbl_names1<-c("varname","model","nparam","AICc","R2m","R2c")
#covartbl_names2<-names(covartbl)[grep("term|coef",names(covartbl))]

#covartbl_names<-c(covartbl_names1,covartbl_names2)
#covartbl<-covartbl %>% dplyr::select(covartbl_names)
covartbl<-covartbl %>% rename(var=varname,k=nparam)

covartbl<-covartbl %>% dplyr::select(var,model,k,AICc,R2m,R2c,
  coef0star,coef0,
  term1,coef1star,coef1,
  term2,coef2star,coef2,
  term3,coef3star,coef3,
  term4,coef4star,coef4,
  term5,coef5star,coef5,
  term6,coef6star,coef6,
  p1,p2,p3,p4,p5,p6,
  interaction1=term4sign,
#  interactions,
  coef0star,coef1star,
  coef2star,coef3star,
  coef4star,coef5star,
...
#if(interact==0){
#  covartbl<-covartbl %>% dplyr::select(var=varname,model,k=nparam,AICc,R2m,R2c,
#                                       coef0star,coef0,
#                                       term1,coef1star,coef1,
#                                       term2,coef2star,coef2,
#                                       term3,coef3star,coef3,
...
desired_order <- data.frame(var=c("Temp","DO","pH","Secchi","Chl","log10Chl","TDN","TDP","SO4","log10Clad","log10Rot","log10Cope","SWE","iceoutd",
  "Cond","Ca","Cl","K","Mg","Na"))#,"Clad","Rot","Cope"))

desired_order$order<-1:length(desired_order[,1])
covartbl_supp0<-merge(desired_order,covartbl,by="var") 
covartbl_supp0<-covartbl_supp0 %>% arrange(order,k,(AICc))

covartbl_supp0<-covartbl_supp0 %>% mutate(b0=coef0)
#covartbl_supp0<-covartbl_supp0 %>% mutate(interaction1=paste(term4,coef4star),b0=coef0)
...
covartbl_supp<-covartbl_supp0 %>% select(var,model,k,AICc,R2m,R2c,b0=coef0,
  term1,m1=coef1star,
  term2,m2=coef2star,
  term3,m3=coef3star,
  term4,m4=coef4star)
#  interaction1)#,
#  interaction1=coef4star)#,interaction2,interaction3)
...
which_dropdups<-which(duplicated(covartbl_supp[,1:13])==TRUE)
covartbl_supp_out<-covartbl_supp[-which_dropdups,]
#covartbl_supp_out$interaction1<-paste("", covartbl_supp_out$interaction1)
write.csv(covartbl_supp_out,file=paste(fileout,"_covar_sig_supp.csv",sep=""),row.names=FALSE,quote=FALSE)
...
desired_order <- data.frame(var=c("Temp","DO","pH","Secchi","Chl","log10Chl","TDN","TDP","SO4","log10Clad","log10Rot","log10Cope",
  "SWE","iceoutd"))
#  "Cond","Ca","Cl","K","Mg","Na"))#,"Clad","Rot","Cope"))
desired_order$order<-1:length(desired_order[,1])
covartbl_main0<-merge(desired_order,covartbl,by="var") 

covartbl_main<-covartbl_main0 %>%  arrange(order,k,(AICc)) %>% 
  dplyr::select(var,model,k,AICc,R2m,R2c,
  coef0,term1,coef1,
  term2,coef2,
  term3,coef3,
  term4,coef4,
...
#covartbl_main$coef4sign<-""
#covartbl_main$coef5sign<-""
#covartbl_main$coef6sign<-""

#covartbl_main$term4<-0
#covartbl_main$term5<-0
#covartbl_main$term6<-0

#covartbl_main$coef4sign[which(covartbl_main$coef4<0)]<-"-"
#covartbl_main$coef4sign[which(covartbl_main$coef4>0)]<-"+"
#covartbl_main$coef5sign[which(covartbl_main$coef5<0)]<-"-"
#covartbl_main$coef5sign[which(covartbl_main$coef5>0)]<-"+"
#covartbl_main$coef6sign[which(covartbl_main$coef6<0)]<-"-"
#covartbl_main$coef6sign[which(covartbl_main$coef6>0)]<-"+"
#covartbl_main$coef4sign[which(as.numeric(covartbl_main$p4)>0.05)]<-""
#covartbl_main$coef5sign[which(as.numeric(covartbl_main$p5)>0.05)]<-""
#covartbl_main$coef6sign[which(as.numeric(covartbl_main$p6)>0.05)]<-""
#covartbl_main$term4[which(as.numeric(covartbl_main$p4)>0.05)]<-""
#covartbl_main$term5[which(as.numeric(covartbl_main$p5)>0.05)]<-""
#covartbl_main$term6[which(as.numeric(covartbl_main$p6)>0.05)]<-""
#covartbl_main$coef4sign[which(covartbl_main$term4=="")]<-""
#covartbl_main$coef5sign[which(covartbl_main$term5=="")]<-""
#covartbl_main$coef6sign[which(covartbl_main$term6=="")]<-""

#covartbl_main<-covartbl_main %>% mutate(interactions=paste(coef4sign,term4," ", coef5sign,term5," ", coef6sign,term6,sep=""))
#covartbl_main<-covartbl_main %>% select(-coef4,-coef5,-coef6,-coef4sign,-coef5sign,-coef6sign,-term4,-term5,-term6,
#  -p1,-p2,-p3,-p4,-p5,-p6)
#covartbl_main<-covartbl_main %>% select(-coef4,-coef4sign,-term4,
#                                        -p1,-p2,-p3,-p4)
#covartbl_main$interactions<-shQuote(covartbl_main$interactions)
covartbl_main$interactions<-paste("",covartbl_main$interaction1)

covartbl_main<-covartbl_main %>% select(-term4,-coef4,-interaction1)
covartbl_main_out<-covartbl_main %>% filter(var!="SO4") # drop sulfate from main table, weak models with non-signif coefs, low r2
write.csv(covartbl_main_out,file=paste(fileout,"_covar_main.csv",sep=""),row.names=FALSE,quote=FALSE)

#######################################

# for comparison, rerun models as multiple regressions (no random effects)
# then subset to all one variable models that had a significant term
...
which_interact<-c(grep("\\*",models_df0$modname),grep("\\^",models_df0$modname)) %>% unique()
...
onevar_all<-rbind(onevar_main,onevar_supp)
write.csv(onevar_main,file=paste(fileout,"_onevar_sig.csv",sep=""),row.names=FALSE,quote=FALSE)
write.csv(onevar_supp,file=paste(fileout,"_onevar_supp_sig.csv",sep=""),row.names=FALSE,quote=FALSE)
write.csv(onevar_all,file=paste(fileout,"_onevar_sig_all.csv",sep=""),row.names=FALSE,quote=FALSE)
...
data_formodel<-data_formodel %>% select(value, park_site,park_code,a=ice_out_doy, b=solar_jas, c=hydro)
data_formodel<-na.omit(data_formodel)
bestmodelformula<-formula(value ~ a + b + c)
mi<-lme(na.action=na.omit, bestmodelformula, random = randompart, method="ML",data=data_formodel) 
midata<-mi$data
midata$preds<-predict(mi) %>% as.numeric()
midata$resid<-residuals(mi) %>% as.numeric()

plotdata<-midata

wtempplot<-ggplot(plotdata, aes(x = a, y = value))+#, color = park_code)) + 
  geom_line(aes(x=a,y=preds,group=park_site),alpha=0.2)+
#  geom_smooth(aes(x=a,y=preds),se=FALSE,color="gray")+
  geom_point(size = ptsize,aes(x = a, y = value,color=park_code)) + # change size and colour
  scale_color_viridis_d(end = 0.85,na.value="gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("ice out doy")+ylab("water temp (deg C)")

# DO 
data_formodel_multi<- data_forstats %>% filter(var_abbrev1 %in% c("DO","Chl","Watertemp")) %>% 
  select(var_abbrev1,park_site,park_code,year,
         parklakenum,short_code,
         value) %>% unique() %>% 
  tidyr::pivot_wider(names_from=var_abbrev1,values_from=value,values_fn = list(value=mean)) %>% as.data.frame()

data_formodel<-data_formodel_multi %>% select(value=DO, park_site,park_code,a=Watertemp, b=Chl)
data_formodel<-na.omit(data_formodel)
bestmodelformula<-formula(value ~ a + b)
mi<-lme(na.action=na.omit, bestmodelformula, random = randompart, method="ML",data=data_formodel) 
midata<-mi$data
midata$preds<-predict(mi) %>% as.numeric()
midata$resid<-residuals(mi) %>% as.numeric()
...
doplot<-ggplot(plotdata, aes(x = a, y = value))+#, color = park_code)) + 
  geom_line(aes(x=a,y=preds,group=park_site),alpha=0.2)+
#  geom_smooth(aes(x=a,y=preds),se=FALSE,color="gray")+
  geom_point(size = ptsize,aes(x = a, y = value, color = park_code)) + # change size and colour
  scale_color_viridis_d(end = 0.85,na.value="gray")+
  ...
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("water temp (deg C)")+ylab("DO (mg/L)")

# pH
data_formodel_multi<- data_forstats %>% filter(var_abbrev1 %in% c("pH")) %>% 
  select(var_abbrev1,park_site,park_code,year,
         parklakenum,short_code,SWE_May_snotel,biotitegneiss,
         value) %>% unique() %>% 
  tidyr::pivot_wider(names_from=var_abbrev1,values_from=value,values_fn = list(value=mean)) %>% as.data.frame()
data_formodel<-data_formodel_multi %>% select(value=pH, park_site,park_code,a=SWE_May_snotel, b=biotitegneiss)
data_formodel<-na.omit(data_formodel)
bestmodelformula<-formula(value ~ a + b)
mi<-lme(na.action=na.omit, bestmodelformula, random = randompart, method="ML",data=data_formodel) 
midata<-mi$data
...
phplot<-ggplot(plotdata, aes(x = a, y = value)) + 
  geom_line(aes(x=a,y=preds,group=park_site),alpha=0.2)+
  geom_point(size = ptsize,aes(x = a, y = value, color = park_code)) + # change size and colour
  scale_color_viridis_d(end = 0.85,na.value="gray")+
  theme_bw()+
...
# Secchi
data_formodel_multi<- data_forstats %>% filter(var_abbrev1 %in% c("Secchi","Chl")) %>% 
  select(var_abbrev1,park_site,park_code,year,
         parklakenum,short_code,SWE_May_snotel,
         value) %>% unique() %>% 
  tidyr::pivot_wider(names_from=var_abbrev1,values_from=value,values_fn = list(value=mean)) %>% as.data.frame()
data_formodel<-data_formodel_multi %>% select(value=Secchi, park_site,park_code,a=SWE_May_snotel, b=Chl)
data_formodel<-na.omit(data_formodel)
bestmodelformula<-formula(value ~ a + b)
mi<-lme(na.action=na.omit, bestmodelformula, random = randompart, method="ML",data=data_formodel) 
midata<-mi$data
...
secchiplot<-ggplot(plotdata, aes(x = a, y = value)) + 
  geom_line(aes(x=a,y=preds,group=park_site),alpha=0.2)+
  geom_point(size = ptsize,aes(x = a, y = value, color = park_code)) + # change size and colour
  scale_color_viridis_d(end = 0.85,na.value="gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("SWE (cm)")+ylab("secchi depth (m)")

# Chl
data_formodel_multi<- data_forstats %>% filter(var_abbrev1 %in% c("Chl","TDN","Watertemp")) %>% 
  select(var_abbrev1,park_site,park_code,year,
         parklakenum,short_code,SWE_May_snotel,
         value) %>% unique() %>% 
  tidyr::pivot_wider(names_from=var_abbrev1,values_from=value,values_fn = list(value=mean)) %>% as.data.frame()
data_formodel<-data_formodel_multi %>% select(value=Chl, park_site,park_code,a=TDN, b=Watertemp)
data_formodel<-na.omit(data_formodel)
bestmodelformula<-formula(value ~ a + b)
mi<-lme(na.action=na.omit, bestmodelformula, random = randompart, method="ML",data=data_formodel) 
midata<-mi$data
...
chlplot<-ggplot(plotdata, aes(x = a, y = value)) + 
  geom_line(aes(x=a,y=preds,group=park_site),alpha=0.2)+
  geom_point(size = ptsize,aes(x = a, y = value, color = park_code)) + # change size and colour
  scale_color_viridis_d(end = 0.85,na.value="gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  ylim(0,3)+
  xlab("TDN (mg/L)")+ylab("chlorophyll (ug/L)")

# TDN
data_formodel_multi<- data_forstats %>% filter(var_abbrev1 %in% c("TDN")) %>% 
  select(var_abbrev1,park_site,park_code,year,
         parklakenum,short_code,SWE_May_snotel,ice_out_doy,forest,
         value) %>% unique() %>% 
  tidyr::pivot_wider(names_from=var_abbrev1,values_from=value,values_fn = list(value=mean)) %>% as.data.frame()
data_formodel<-data_formodel_multi %>% select(value=TDN, park_site,park_code,a=forest, b=ice_out_doy)
#data_formodel$dummy<-1
data_formodel<-na.omit(data_formodel)
bestmodelformula<-formula(value ~ a + b)
mi<-lme(na.action=na.omit, bestmodelformula, random = randompart, method="ML",data=data_formodel) 
midata<-mi$data
...
tdnplot<-ggplot(plotdata, aes(x = a, y = value)) + 
  geom_line(aes(x=a,y=preds),se=FALSE,color="gray")+
  geom_point(size = ptsize,aes(x = a, y = value, color=park_code)) + # change size and colour
  scale_color_viridis_d(end = 0.85,na.value="gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("% forest")+ylab("TDN (mg/L)")

# clad
data_formodel_multi<- data_forstats %>% filter(var_abbrev1 %in% c("clad","Watertemp")) %>% 
  select(var_abbrev1,park_site,park_code,year,
         parklakenum,short_code,SWE_May_snotel,ice_out_doy,
         value) %>% unique() %>% 
  tidyr::pivot_wider(names_from=var_abbrev1,values_from=value,values_fn = list(value=mean)) %>% as.data.frame()
data_formodel<-data_formodel_multi %>% select(value=clad, park_site,park_code,a=ice_out_doy, b=Watertemp)
data_formodel$value<-log10(data_formodel$value+1)
data_formodel<-na.omit(data_formodel)
bestmodelformula<-formula(value ~ a + b)
mi<-lme(na.action=na.omit, bestmodelformula, random = randompart, method="ML",data=data_formodel) 
midata<-mi$data
...
cladplot<-ggplot(plotdata, aes(x = a, y = value)) + 
  geom_line(aes(x=a,y=preds,group=park_site),alpha=0.2)+
  geom_point(size = ptsize,aes(x = a, y = value, color = park_code)) + # change size and colour
  scale_color_viridis_d(end = 0.85,na.value="gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  ylim(0,5)+
  xlab("ice out doy")+ylab("log10 cladocerans (ind/L)")


gridplot<-grid.arrange(wtempplot,secchiplot,phplot,chlplot,tdnplot,cladplot,nrow=2)
gridplot

ggsave(filename = "../figures/analysispowers/datamodel_multi.png",
       plot = gridplot, width = 8, height = 5, units = "in")
##############################################################
# ice out doy and swe only
...
data_formodel_multi<- data_forstats %>% filter(var_abbrev1 %in% c("DO","Chl","Watertemp")) %>% 
  select(var_abbrev1,park_site,park_code,year,
         parklakenum,short_code,ice_out_doy,SWE_May_snotel,
         value) %>% unique() %>% 
  tidyr::pivot_wider(names_from=var_abbrev1,values_from=value,values_fn = list(value=mean)) %>% as.data.frame()

data_formodel<-data_formodel_multi %>% select(value=DO, park_site,park_code,a=Watertemp, b=Chl, c=ice_out_doy)
data_formodel<-na.omit(data_formodel)
bestmodelformula<-formula(value ~ a + b + c)
mi<-lme(na.action=na.omit, bestmodelformula, random = randompart, method="ML",data=data_formodel) 
midata<-mi$data
...
doplot<-ggplot(plotdata, aes(x = c, y = value))+#, color = park_code)) + 
  geom_line(aes(x=c,y=preds,group=park_site),alpha=0.2)+
  geom_point(size = ptsize,aes(x = c, y = value, color = park_code)) + # change size and colour
  scale_color_viridis_d(end = 0.85,na.value="gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("ice out doy")+ylab("DO (mg/L)")
...
data_formodel_multi<- data_forstats %>% filter(var_abbrev1 %in% c("Chl","TDN","Watertemp")) %>% 
  select(var_abbrev1,park_site,park_code,year,
         ...
         value) %>% unique() %>% 
  tidyr::pivot_wider(names_from=var_abbrev1,values_from=value,values_fn = list(value=mean)) %>% as.data.frame()
data_formodel<-data_formodel_multi %>% select(value=Chl, park_site,park_code,a=TDN, b=Watertemp, c=SWE_May_snotel)
data_formodel<-na.omit(data_formodel)
bestmodelformula<-formula(value ~ a + b + c)
mi<-lme(na.action=na.omit, bestmodelformula, random = randompart, method="ML",data=data_formodel) 
midata<-mi$data
...
chlplot<-ggplot(plotdata, aes(x = a, y = value)) + 
  ...
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  ylim(0,3)+
  xlab("SWE (cm)")+ylab("chlorophyll (ug/L)")
...
tdnplot<-ggplot(plotdata, aes(x = b, y = value)) + 
  geom_line(aes(x=b,y=preds,group=park_site),alpha=0.25)+
  geom_point(size = ptsize,aes(x = b, y = value, color=park_code)) + # change size and colour
  scale_color_viridis_d(end = 0.85,na.value="gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("ice out doy")+ylab("TDN (mg/L)")
...
ggsave(filename = "../figures/analysispowers/datamodel_multi_iceswe.png",
       plot = gridplot, width = 8, height = 5, units = "in")
...
data_forstats %>% select(park_site, andesite) %>% unique()
  
data_forstats %>% select(park_site, basalt) %>% unique()

data_forstats %>% select(park_site, basalt) %>% unique()


data_forstats %>% select(park_site, andesite, basalt, biotitegneiss, granodiorite, graywacke, quartzmonzodiorite,
                         quartzmonzonite, sandstone) %>% unique()
