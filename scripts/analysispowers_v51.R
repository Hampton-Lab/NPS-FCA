...
models_top_signif_df<-models_df %>% filter(term!= "(Intercept)" & p.value<=0.25) %>% dplyr::select(varname,modname,modname2) %>% unique()
...
covartbl_main$interactions<-paste(" ",covartbl_main$interactions)
covartbl_main$interactions

write.csv(covartbl_main,file=paste(fileout,"_covar_main.csv",sep=""),row.names=FALSE,quote=FALSE)
