...
ggplot(plotdata, aes(x = a, y = value, color = park_code)) + 
  geom_point(size = 3) + # change size and colour
  geom_line(aes(x=a,y=preds,group=park_site))+
...
ggplot(plotdata, aes(x = b, y = value, color = park_code)) + 
  geom_point(size = 3) + # change size and colour
  geom_line(aes(x=b,y=preds,group=park_site))+
  scale_color_viridis_d(end = 0.85,na.value="dark gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
ggplot(plotdata, aes(x = c, y = value, color = park_code)) + 
  geom_point(size = 3) + # change size and colour
  geom_line(aes(x=c,y=preds,group=park_site))+
  scale_color_viridis_d(end = 0.85,na.value="dark gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
data_formodel<-na.omit(data_formodel)
bestmodelformula<-formula(value ~ a + b + c)
bestmodelformula_a<-formula(value ~ a)
mi<-lme(na.action=na.omit, bestmodelformula, random = randompart, method="ML",data=data_formodel) 
mia<-lme(na.action=na.omit, bestmodelformula_a, random = randompart, method="ML",data=data_formodel) 

midata<-mi$data
miadata<-mia$data
midata$preds<-predict(mi) %>% as.numeric()
miadata$preds<-predict(mia) %>% as.numeric()
miadata$resida<-residuals(mia) %>% as.numeric()


plotdata<-cbind(midata,preda=miadata$preds,resida=miadata$resida)

ggplot(plotdata, aes(x = a, y = value, color = park_code)) + 
  geom_point(size = 3) + # change size and colour
  geom_line(aes(x=a,y=preda,group=park_site))+
  scale_color_viridis_d(end = 0.85,na.value="dark gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
ggplot(plotdata, aes(x = resida, y = b, color = park_code)) + 
  geom_point(size = 3) + # change size and colour
#  geom_line(aes(x=a,y=resida,group=park_site))+
  scale_color_viridis_d(end = 0.85,na.value="dark gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
raneff<-ranef(mi) %>% as.data.frame()
raneff$park_site<-row.names(raneff)
names(raneff)[which(names(raneff)=="(Intercept)")]<-"RandInt"
fixeff<-fixef(mi) %>% as.data.frame() %>% t() %>% as.data.frame()
names(fixeff)<-c("Int","ma","mb","mc")

merge1<-merge(midata,raneff,by.x="park_site",by.y="park_site")
merge2<-cbind(merge1,fixeff)
data_forplot<-merge2
data_forplot$preds<-data_forplot$Int + data_forplot$RandInt + 
  data_forplot$a*data_forplot$ma + data_forplot$b*data_forplot$mb + 
  data_forplot$c*data_forplot$mc

data_forplot$preda<-data_forplot$Int + data_forplot$RandInt + 
  data_forplot$a*data_forplot$ma 

ggplot(data_forplot, aes(x = a, y = value, color = park_code)) + 
  geom_point(size = 3) + # change size and colour
  geom_line(aes(x=a,y=preds,group=park_site))+
  scale_color_viridis_d(end = 0.85,na.value="dark gray")+
  theme_bw()+
...
ggplot(dataplot, aes(x = a, y = value, color = park_code)) + 
  geom_point(size = 3) + # change size and colour
  geom_line(aes(x=a,y=preda,group=park_site))+
  scale_color_viridis_d(end = 0.85,na.value="dark gray")+
  theme_bw()+
...
data_formodel<-data_formodel %>% select(value, park_site,park_code,a=SWE_May_snotel, b=biotitegneiss)
data_formodel<-na.omit(data_formodel)
bestmodelformula<-formula(value ~ a + b)
mi<-lme(na.action=na.omit, bestmodelformula, random = randompart, method="ML",data=data_formodel) 
#data_formodel$predicta<-coef(mi)$'(Intercept)'+coef(mi)$a * data_formodel$a 
#data_formodel$predicta<-predict(mi) %>% as.numeric()
...
data_wtemp<- data_forstats %>% filter(var_abbrev1=="Watertemp")
data_wtemp<-data_wtemp %>% select(value, park_site,park_code,a=ice_out_doy, b=solar_jas, c=hydro)
data_ph<- data_forstats %>% filter(var_abbrev1=="pH")
data_ph<-ph %>% select(value, park_site,park_code,a=ice_out_doy, b=solar_jas, c=hydro)
data_wtemp<- data_forstats %>% filter(var_abbrev1=="Watertemp")
data_wtemp<-data_wtemp %>% select(value, park_site,park_code,a=ice_out_doy, b=solar_jas, c=hydro)

watertemp<-ggplot(data_wtemp, aes(x = a, y = value, col=park_code))+
  geom_point()+
  scale_color_viridis_d(end = 0.85,na.value="dark gray")+
  ylab("Water temp")+
  xlab("Ice out doy")+
...
ph<-ggplot(data_formodel, aes(x = a, y = value, col=park_code))+
  geom_point()+
  scale_color_viridis_d(end = 0.85,na.value="dark gray")+
  ylab("Water temp")+
  xlab("Ice out doy")+
...
ggplot(data_formodel, aes(x = a, y = value, color = park_code) ) +
  geom_point() +
...
p <- ggplot(data_formodel, aes(x = a, y = value, colour = park_code)) +
  geom_point(size=3) +
  geom_line(aes(y = predict(mi)),size=1)+
  
print(p)


ggplot(data_formodel, aes(x = a, y = value, col=park_code))+
  geom_point()+
  geom_line(aes(x=a,y=predict))+
  scale_color_viridis_d(end = 0.85,na.value="dark gray")+
  theme_bw()+
  theme(legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
daterr0<-data_formodel
daterr0$fitted_1stvar<-as.numeric(predict(mi_1stvar))
daterr0$resid_1stvar<-as.numeric(residuals(mi_1stvar))
