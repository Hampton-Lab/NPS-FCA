...
sitestable<-sitestable %>% rename(Elev_m=Elevation_m,Depthmax_m=Depth_max,Depthmean_m=Depth_mean_m,Shorelength_m=Shoreline_length_m,SurfArea_ha=Surface_area_ha,ShedArea_ha=Watershed_area_ha,Vol_m3=Volume_m3,ShedAreaLakeArea_ratio=Watershed_area_to_lake_area_ratio)
...
temper_main<-one %>% mutate_if(is.numeric, signif,digits=2) %>% select(-df,-statistic,-effect,-group,variable,term,model,val=estimate,se=std.error,pval=p.value,sig)
temper_main$term[which(temper_main$term=="(Intercept)")]<-"Intercept"

temper_interact<-rbind(two,three,four,five,six,seven)
temper_interact$term[which(temper_interact$term=="(Intercept)")]<-"Intercept"
temper_interact$sig<-""
temper_interact$sig[which(temper_interact$p.value<=0.05)]<-"*"
temper_interact$sig[which(temper_interact$p.value<=0.01)]<-"**"
temper_interact$sig[which(temper_interact$p.value<=0.005)]<-"***"
#temper_interact<-temper_interact[which(temper_interact$term!="(Intercept)"),]

temper_interact<-temper_interact %>% mutate_if(is.numeric, signif,digits=2) %>% select(-df,-statistic,-effect,-group,variable,term,model,val=estimate,se=std.error,pval=p.value,sig)
temper_interact<-temper_interact[order(temper_interact$variable,temper_interact$term),]
...
yr_interact_coef<-subset(temper_interact,substr(temper_interact$term,1,2)=="yr")
int_interact_coef<-subset(temper_interact,substr(temper_interact$term,1,2)=="In") %>% select(-term)
temper_interact_wide<-merge(int_interact_coef,yr_interact_coef,by=c("variable","model"),suffixes=c(".int",".coef"))
