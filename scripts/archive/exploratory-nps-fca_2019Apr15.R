...
waterchem_samples <- read_excel(
  path = file.path("..",
                   "NPS_NCCN_Mtn_Lakes_Exports_20180917",
                   "qs_b334_Water_Chemistry_Samples_select_20180917_170253.xlsx")) %>% as.data.frame()

waterchem_data <- read_excel(
  path = file.path("..",
                   "NPS_NCCN_Mtn_Lakes_Exports_20180917",
                   "qs_b344_Water_Chemistry_Data_select_20180917_170340.xlsx")) %>% as.data.frame()
...
write.csv(subset(missing.surface,missing.surface$missingTF==1),"../missing_surfwaterchem.csv")
write.csv(subset(missing.mid,missing.mid$missingTF==1),"../missing_midwaterchem.csv")
...
# Which lakes have we been given in main dataset?
all_excel <- list.files(path = "../NPS_NCCN_Mtn_Lakes_Exports_20180917/",
           pattern = "*.xlsx", full.names = T)

file_contents <- sapply(X = all_excel, FUN = read_excel, simplify = FALSE)

site_names <- lapply(X = file_contents, FUN = select, Site_name)

names(site_names) <- NULL

site_names <- unlist(site_names)
...
# Look at the continuous temp data
all_con_temp <- list.files(path = "../FCA_ContinuousTempData", recursive = T,
                           pattern = "*.csv", full.names = T)

temp_file_contents <- sapply(X = all_con_temp, FUN = fread,
...
length(all_con_temp)
length(temp_file_contents)


short_names <- gsub(x = all_con_temp,
                    pattern = "../FCA_ContinuousTempData/FCA_ContinuousTempData/",
                    replacement = "")

all_temp_merge <- map2_df(temp_file_contents, short_names,
                    ~update_list(.x, location = .y))
...
all_temp_merge <- all_temp_merge %>%
  mutate(date_time = as.POSIXct(x = date_time, format = "%d/%m/%Y %H:%M:%S"))

head(all_temp_merge)

lake_measures <- ggplot(data = all_temp_merge) +
  geom_point(aes(x = date_time, y = value, group = lake, color = park),
             alpha = 0.10) +
  facet_wrap(~ measure)

ggsave(filename = "../figures/all-lakes-and-measures.png", plot = lake_measures,
       device = "png", width = 12, height = 8, units = "in")

# short_split_names <- ldply(strsplit(x = short_names, split = "/")) 


# bind_data_names <- mapply(FUN = list, temp_file_contents, short_split_names,
#                           SIMPLIFY = FALSE)
# 
# 
# lapply(X = bind_data_names, FUN = cbind)
# 
# temp_file_contents
# 
# cbind(temp_file_contents[1], short_split_names[1, ])
#   
# bind.ith.rows <- function(i) do.call(cbind, lapply(sample.list, "[", i, TRUE))
# 
# bind.ith.rows(1)
# 
# 
