...
# 
vis.agg<-vis %>%
  select(Park_code, Site_name, Location_code, Fish_taxon_code,Fish_life_stage) %>%
  group_by(Park_code, Site_name, Location_code,Fish_taxon_code,Fish_life_stage) %>%
  unique() %>%
  tally() %>% as.data.frame()

unique(vis.agg$Site_name)
