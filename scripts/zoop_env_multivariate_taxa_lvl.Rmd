...
A script for zooplankton community analysis using the "taxa" groupings from the
NPS dataset.
...
filter out taxa providing <5% of a sample before making into a matrix.
```{r}
zoop_prop_taxa <- zoop %>%
...
Should sum to 1 within rows: (isn't exact...rounding issue?)
```{r}
...
Run another NDMS, this time on the version of the community matrix that made it into the dbRDA. It's a subset of the full one used for the earlier nmds_2:
```{r results="hide"}
