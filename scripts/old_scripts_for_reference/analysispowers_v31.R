...
#bigjoin.stats$ice_out_doy<-yday(bigjoin.stats$start_date)-bigjoin.stats$ice_out_doy
...
sitestable<-sitestable %>% dplyr::rename(Park=park_code,Lake=short_code,
  ...
  select(Park,Lake,Elev_m,Elevwshed_m,
         ...
         snowice,forest,barren,hydro,SWE,iceout_doy,
         bst,btg,grd,grw,qzm) %>% 
...
mod_covar<-function(covars,data,varname,valuecol,fixedpart,randompart,doplots){
  for (i in 1:length(covars[1,])){
    ...
    covarsi<-covars[,i];
    daterr<-daterr %>% select(value_model,covarsi,park_site,park_code,year,snowyr,SWE_May_snotel,SWE_May_snotel_model,ice_out_doy,ice_out_doy_model) %>% na.omit()
    daterr<-daterr %>% unique()
    daterr$nothing<-1

    daterr$SWE_May_snotel<-daterr$SWE_May_snotel_model
    daterr$ice_out_doy<-daterr$ice_out_doy_model
...
#    mi<-rsquared(mi)
  ...
  daterr<-daterr[,which(names(daterr) %in% c("value_model","park_site","park_code","year","snowyr","SWE_May_snotel","SWE_May_snotel_z",
                                             "parklakenum","short_code","SWE_May_snotel_model","ice_out_doy_model",bestmodel_covars))]
  ...
  maxxy<-max(c(daterr_wallgrp$value_model,daterr_wallgrp$fitted),na.rm=TRUE)
  minxy<-min(c(daterr_wallgrp$value_model,daterr_wallgrp$fitted),na.rm=TRUE)
...
if(doplots==1) {
  ...
  plotit<-ggplot(daterr_wallgrp, aes(x = fitted, y = value_model, col=park_site2))+#,group=nothing)) +

    geom_point(alpha = 0.5,daterr_wallgrp, mapping=aes(x = fitted, y = value_model,shape=snowyr))+
    ...
    ylab(paste("Observed",varname))+
    xlab(paste("Predicted",varname))+
  ...
  for(j in 1:length(bestmodel_covars)){
    ...
    plotit1<-ggplot(daterr, aes(x = var, y = value_model, col=park_site))+#,shape=snowyr)) +
      geom_line(data = cbind(daterr, y.hat = predict(mi)), aes(x = var, y = y.hat),alpha=0.5) + 
      geom_point(alpha = 0.5,aes(x = var, y = value_model, col=park_site,shape=snowyr))+
      guides(shape=FALSE)+
      theme_bw()+
  ...
  plotit1b<-ggplot(daterr_wallgrp, aes(x = var, y = value_model, col=park_site))+#,shape=snowyr)) +
    geom_line(data = cbind(daterr, y.hat = predict(mi)), aes(x = var, y = y.hat),alpha=0.5) + 
    geom_point(alpha = 0.5,aes(x = var, y = value_model, col=park_site2))+#,shape=snowyr))+
  ...
  plotit1c<-ggplot(daterr, aes(x = var, y = value_model,col=park_site,label=short_code))+#, col=park_site))+#,shape=snowyr)) +
    geom_line(data = cbind(daterr, y.hat = predict(mi)), aes(x = var, y = y.hat),alpha=0.5) + 
    geom_text(aes(x = var, y = value_model),size=2,alpha=1)+#, col=park_site))+#,shape=snowyr))+
  ...
  mi_simpler<-glm(na.action=na.omit, value_model~var1, data=daterr) #%>% as.data.frame() #%>% filter(effect!="ran_pars")# %>% as.data.frame() %>% mutate(model=paste("2_",i,sep=""))
  if(usenlme==1){
    mi_simpler<-lme(na.action=na.omit, value_model~var1, random = randompart, method="ML",data=daterr) #%>% as.data.frame() #%>% filter(effect!="ran_pars")# %>% as.data.frame() %>% mutate(model=paste("2_",i,sep=""))
  }
...
#  mi_simpler<-lm(na.action=na.omit, value_model~varA, data=daterr) #%>% as.data.frame() #%>% filter(effect!="ran_pars")# %>% as.data.frame() %>% mutate(model=paste("2_",i,sep=""))
  #mi_simpler<-lm(na.action=na.omit, value_model~varA+varB, data=daterr) #%>% as.data.frame() #%>% filter(effect!="ran_pars")# %>% as.data.frame() %>% mutate(model=paste("2_",i,sep=""))
  
  daterr0<-daterr
  daterr0$fitted<-fitted(mi_simpler)
  daterr0$resid<-resid(mi_simpler)
  daterrA<-daterr0
  daterrB<-daterr0
  daterrC<-daterr0
  
  daterrA$X<-daterr0$varA
  daterrA$Y<-daterr0$value_model
  daterrA$panel<-"A.  Y ~ X1"
  daterrB$X<-daterr0$varB
  daterrB$Y<-daterr0$resid
...
#    stat_smooth(geom='line', method="lm",alpha=alph, se=FALSE)+
    geom_line(data = cbind(daterrA, y.hat = predict(mi_simpler)), aes(x = X, y = y.hat),alpha=0.5) + 
 ...
 # plotit1<-ggplot(mi_resids) + 
 #   geom_point(aes(x=.varA, y=.resid))
