...
# assign iceout_doy dummy variables
...
#modelworthyvars<-m_0covar_allvar_out[unique(c(grep("\\*",m_0covar_allvar_out$Mlo),grep("\\*",m_0covar_allvar_out$Mhi))),]$variable
...
###########################
mod_covar<-function(covars,data,varname,valuecol,fixedpart,randompart,doplots=1){
  for (i in 1:length(covars[1,])){
    ...
    daterr$nothing<-1
    daterr<-daterr[complete.cases(daterr),]
    ...
    modelformula_lmer<-formula(paste("value_model~",fixedpart_named,"+",randompart,sep=""))
...
#    mi<-summary(mi)$tTable %>% as.data.frame()
    ...
    if(usenlme==1){
#    modelformula<-formula(modelformula+randompart)
    mi<-lmer(modelformula_lmer,data=daterr) #%>% as.data.frame() #%>% filter(effect!="ran_pars")# %>% as.data.frame() %>% mutate(model=paste("2_",i,sep=""))
    
    models <- dredge(mi, rank = 'AICc',extra="adjR^2") %>% print()
    top_models <- subset(models, delta < 10) 
    ave_models <- model.avg(top_models)
    ave_models_summary<-summary(ave_models)
    ave_modelsimp<-importance(ave_models_summary)
    mostimp<-names(ave_modelsimp)[-grep(":",names(ave_modelsimp))]
    ...
    covars_ordered<-row.names(mi_coef)[1+c(1:length(covarsi))]
    miR<-r.squaredGLMM(mi)[1]
    
    mi0<-lme(na.action=na.omit, formula(value_model ~1), random = randompart, method="ML",data=daterr) #%>% as.data.frame() #%>% filter(effect!="ran_pars")# %>% as.data.frame() %>% mutate(model=paste("2_",i,sep=""))

    da.lmm<-dominanceAnalysis(mi, null.model=mi0)
    }
...
#    covars_ordered<-names(mi$coefficients)[1+c(1:length(covarsi))]
    # try sorting main effect variables by relative importance and rerun model
    # but skip it if relaimpo compution is singular
#    if(length(covarsi)>1){
#    mi.relaimpo<-try(calc.relimp(mi),TRUE)
#    if(class(mi.relaimpo)!="try-error"){
#    mi.relaimpo.lmg<-mi.relaimpo$lmg
#    grep(":",names(mi.relaimpo$lmg))
#    mi.relaimpo.lmg.main<-names(mi.relaimpo$lmg)[1:length(covarsi)]#[-grep(":",names(mi.relaimpo$lmg))]
 #   newform<-paste(mi.relaimpo.lmg.main,collapse=" + ")
#    if(length(grep("\\^2",fixedpart_named))>0){
#      fixedpart_named<-paste("(", newform,")^2",sep="")
#    }
#    if(length(grep("\\^2",fixedpart_named))==0){
#      fixedpart_named<-newform
#    }
#    modelformula<-formula(paste("value_model~",fixedpart_named,sep=""))
#    mi<-glm(na.action=na.omit, modelformula, data=daterr) #%>% as.data.frame() #%>% filter(effect!="ran_pars")# %>% as.data.frame() %>% mutate(model=paste("2_",i,sep=""))
#    }
...
#                mapping=aes(x = fitted, y = value_model),col="dark gray")+
...
#  png(filename = "test.png", width = 7, height = 7, units = "in",res=300)
  print(plotit)
  dev.off()
...
#    aes(color=park_site2)+
  ...
  plotit1b<-ggplot(daterr, aes(x = var, y = value_model,col=park_site,label=short_code))+#, col=park_site))+#,shape=snowyr)) +
    geom_line(data = cbind(daterr, y.hat = predict(mi)), aes(x = var, y = y.hat),alpha=0.5) + 
    geom_text(aes(x = var, y = value_model),size=2,alpha=1)+#, col=park_site))+#,shape=snowyr))+
    #    facet_wrap(~substr(park_site,1,4)) +
    facet_wrap(~substr(park_code,1,4)) +
    #aes(shape=parklakenum)+
    scale_colour_viridis_d(end = 0.85)+
    theme(legend.position="none")+
    guides(shape=FALSE,color=FALSE)+
    theme_bw()+
