...
axis_label_tbl <- tribble(
  ...
  "SWE_May", "May SWE (cm)",
  "SWE_May_snotel", "May SWE snotel (cm)",
...
iceout_SWE<-iceout_SWE %>% group_by(short_code) %>% mutate(SWE_rank = dense_rank((SWE_May))/max(dense_rank((SWE_May))),
                                                           SWE_rank_snotel = dense_rank(SWE_May_snotel)/max(dense_rank(SWE_May_snotel),na.rm=TRUE)) %>% as.data.frame()
...
iceout_SWE_snotel_plot<-ggplot(data=iceout_SWE,aes(y=SWE_rank_snotel,x=SWE_May_snotel,color=park_code,label=year_label))+
  geom_point(size=4,alpha=0.5)+
  ylab("Cumulative Distribution")+xlab("May SWE snotel (cm) and corresponding ice-out date (day of year)")+
  geom_text(data=iceout_SWE,aes(y=SWE_rank_snotel,x=SWE_May_snotel),color="black",size=3)+
  geom_point(data=iceout_SWE,aes(y=SWE_rank_snotel,x=ice_out_doy,color=park_code),shape=0,size=2)+
  theme_bw()+
  theme(legend.position="none")+
  theme(strip.text.x=element_text(),axis.title.x = element_text(vjust=-0.5))+facet_wrap(~park_siteshort,nrow=3)

ggsave(filename = "../figures/analysispowers/iceout_SWE_snotel_plot.png",
       plot = iceout_SWE_snotel_plot, width = 8, height = 5, units = "in")

snodas_snotel_compare<-bigjoin.stats %>% select(Lake,park_code,park_site,short_code,year,SWE_May,SWE_May_snotel) %>% as.data.frame()
...
#  facet_wrap(~park_site)+
  facet_wrap(~substr(Lake,1,14),ncol=6)+
  theme_bw()+
  theme(legend.position="none")+
...
snodas_wtemp_compare<-bigjoin.stats  %>% filter(variable=="ProfTemp_top2m") %>% select(Lake,axis_label,park_code,park_site,short_code,year,SWE_May,SWE_May,value) %>% as.data.frame()
...
snotel_wtemp_compare<-bigjoin.stats  %>% filter(variable=="ProfTemp_top2m") %>% select(Lake,axis_label,park_code,park_site,short_code,year,SWE_May,SWE_May_snotel,value) %>% as.data.frame()
...
data_forstats<- bigjoin.stats %>% #mutate(snowhi=year==2011,snowlo=year==2015,snowrest=!year %in% c(2011,2015)) %>% 
  ...
  select(variable,axis_label,year,short_code,park_code,park_site,park_siteshort,Depth_mean_m,Volume_m3,Watershed_area_ha,Watershed_area_to_lake_area_ratio,Elevation_m,#Asp,
         ...
         value,
         ice_out_doy,SWE_May,SWE_May_snotel,flush_index_SWE_May,BlueLineInlet,BlueLineOutlet,hydro,
         salmo_adult_ever,
...
minyears<-data_forstats %>% filter(variable=="ProfTemp_top2m",value>-99,year>2008) %>% 
  ...
  dplyr::summarize(min_SWE_May=min(SWE_May_snotel,na.rm=TRUE),minYear_SWE_May=year[which(SWE_May_snotel == min(SWE_May_snotel,na.rm=TRUE))][1]) %>% as.data.frame()
maxyears<-data_forstats %>% filter(variable=="ProfTemp_top2m",value>-99) %>% 
  select(year,short_code,park_code,park_site,park_siteshort,value,SWE_May_snotel) %>%
  group_by(park_site,short_code) %>%
  dplyr::summarize(max_SWE_May=max(SWE_May_snotel,na.rm=TRUE),maxYear_SWE_May=year[which(SWE_May_snotel == max(SWE_May_snotel,na.rm=TRUE))][1]) %>% as.data.frame()
...
data_forstats$snowyear<-"mid"
data_forstats$snowyear[which(data_forstats$snowyrlo==1)]<-"lo"
data_forstats$snowyear[which(data_forstats$snowyrhi==1)]<-"hi"
data_forstats$snowyear <- factor(data_forstats$snowyear, levels = c("mid", "lo", "hi"))
...
vars<-c("snowyear","Depth_mean_m","Volume_m3","Watershed_area_ha","Elevation_m",
        "SWE_May",#"SWE_Apr",
...
vars_1covar<-vars[-which(vars %in% c("snowyear"))] 
vars_2covar<-combn(vars[-which(vars %in% c("yr","park_code","snowyear","SWE_May","flush_index_SWE_May"))],2)
...
mi<-by_variablei %>% group_by(variable) %>% do(tidy(lme(value ~ snowyear, random = ~ 1|park_site,  method="ML",data=.))) %>% as.data.frame() %>% filter(effect!="ran_pars") %>% as.data.frame() %>% mutate(model=1)
mi_glance<-do(by_variablei, glance(lme(value ~ snowyear, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
...
m_0covar_allvar_slope_snowyrlo<-subset(m_0covar_allvar,m_0covar_allvar$term=="snowyearlo") %>% select(-term,-model,-sigma,-logLik,-AIC,-BIC,-pval)# %>% dplyr::rename(val.lo=val,se.lo=se)
m_0covar_allvar_slope_snowyrhi<-subset(m_0covar_allvar,m_0covar_allvar$term=="snowyearhi") %>% select(-term,-model,-sigma,-logLik,-AIC,-BIC,-pval)# %>% dplyr::rename(val.lo=val,se.lo=se)
...
lme_detail<- lme(value ~ snowyear, random = ~ 1|park_site,  method="ML",data=data_forstats_detail) 
e_detail<-allEffects(lme_detail)[[1]] %>% as.data.frame()
e_detail$snowyear<-factor(e_detail$snowyear, levels = c("mid", "lo", "hi"))
e_detail<-melt(e_detail)
e_detail$short_code<-"CR";e_detail$park_code<-"OLYM";e_detail$yearlab<-"10"
e_detail$x<-0.5
e_detail$x[which(e_detail$snowyear=="mid")]<-0.19
e_detail$x[which(e_detail$snowyear=="hi")]<-0.81

  
# do initial ggrepel plot
one<-ggplot(data_forstats_detail,aes(y=value,x=snowyear,color=park_code,label=short_code))+
#  geom_point()+
  geom_text_repel(size=3,segment.color=NA,box.padding=0.3)+#size=2.5,segment.color="black",alpha=1,segment.size = 0.2) + 
#  geom_jitter(size=3)+#size=2.5,segment.color="black",alpha=1,segment.size = 0.2) + 
  ylab(data_forstats_detail$axis_label[1])+
  xlab("Snow year")+
  theme_bw()+theme(legend.position="none",panel.grid = element_blank())

# get the repelled coords

one
grid.force()

kids <- childNames(grid.get("textrepeltree", grep = TRUE))
#kids <- childNames(grid.get("position_jittertree", grep = TRUE))
...
data_wcoords<-data_forstats_detail
data_wcoords$x<-d$x

# one+ggplot(data_forstats_detail,label=year_code)

twoA<-
  ggplot(data_wcoords,aes(y=value,x=x,color=park_code,label=short_code))+
  geom_text(size=2.5)+#test,aes(y=value,x=x))
  ylab(data_forstats_detail$axis_label[1])+
  xlab("Snow year")+
  theme_bw()+
  theme(legend.position = "none",
  ...
  scale_x_continuous(breaks=c(.19,.5,.81), labels=c("mid","lo","hi"), limits=c(0,1))#, trans)
twoB<-twoA+
  geom_point(e_detail %>% filter(variable=="fit"),mapping=twoA$mapping,size=3,color="gray")+
  geom_line(e_detail  %>% filter(snowyear=="mid" & variable %in% c("lower","upper")),mapping=twoA$mapping,size=1,color="gray")+
  geom_line(e_detail  %>% filter(snowyear=="lo" & variable %in% c("lower","upper")),mapping=twoA$mapping,size=1,color="gray")+
  geom_line(e_detail  %>% filter(snowyear=="hi" & variable %in% c("lower","upper")),mapping=twoA$mapping,size=1,color="gray")
detail_wfit_sites<-twoB

threeA<-
  ggplot(data_wcoords,aes(y=value,x=x,color=park_code,label=yearlab))+
  geom_text(size=2.5)+#test,aes(y=value,x=x))
  ylab(data_forstats_detail$axis_label[1])+
  xlab("Snow year")+
  theme_bw()+
...
threeB<-threeA+
  ...
  geom_line(e_detail  %>% filter(snowyear=="lo" & variable %in% c("lower","upper")),mapping=twoA$mapping,size=1,color="gray")+
  geom_line(e_detail  %>% filter(snowyear=="hi" & variable %in% c("lower","upper")),mapping=twoA$mapping,size=1,color="gray")
detail_wfit_years<-threeB

png(filename = "../figures/analysispowers/wtemp_wfit_sitesyears_multi.png",
    width = 8, height = 6, units = "in",res=300)
grid.arrange(detail_wfit_sites,detail_wfit_years)
dev.off()
...
residfit_bysnowyr<-plot(lme_detail, resid(., type = "p") ~ fitted(.) | snowyear, abline = 0)
residbplot<-plot(lme_detail, snowyear ~ resid(.))
valuefit_bysnowyr<-plot(lme_detail, value ~ fitted(.) | snowyear, abline = c(0,1))
...
m_1covar_mid$snowyear<-"mid";m_1covar_hi$snowyear<-"hi";m_1covar_lo$snowyear<-"lo"
...
m_1covar<-rbind(m_1covar_mid,m_1covar_hi,m_1covar_lo) %>% select(snowyear,variable,model,term,val=estimate,std.error,pval=p.value,AICc)
m_1covar_minAICc<-m_1covar %>% group_by(variable,snowyear) %>% 
  dplyr::summarize(minAICc=min(AICc))
m_1covar<-merge(m_1covar,m_1covar_minAICc,by=c("variable","snowyear"))
...
widestats_snowyrhi<-relate_sig %>% filter(snowyear=="hi") %>%
  group_by(term) %>% dplyr::summarise(vars = paste(signvar, collapse=", ")) %>% as.data.frame()
widestats_snowyrlo<-relate_sig %>% filter(snowyear=="lo") %>%
  group_by(term) %>% dplyr::summarise(vars = paste(signvar, collapse=", ")) %>% as.data.frame()
widestats_snowyrmid<-relate_sig %>% filter(snowyear=="mid") %>%
  group_by(term) %>% dplyr::summarise(vars = paste(signvar, collapse=", ")) %>% as.data.frame()
...
m_2covar_mid$snowyear<-"mid";m_2covar_hi$snowyear<-"hi";m_2covar_lo$snowyear<-"lo"
...
# combine stats parts
m_2covar<-rbind(m_2covar_mid,m_2covar_hi,m_2covar_lo) %>% select(snowyear,variable,model,term,val=estimate,std.error,pval=p.value,AICc)
m_2covar_minAICc<-m_2covar %>% group_by(variable,snowyear) %>% 
  dplyr::summarize(minAICc=min(AICc))
m_2covar<-merge(m_2covar,m_1covar_minAICc,by=c("variable","snowyear"))
...
plot_wtemp_flush<-ggplot(dataplot, aes(x=flush_index_SWE_May, y=value, group="black", color=(park_code),label=short_code)) +
  annotation_logticks(sides="b",color="gray",size=.3)+
  geom_line(aes(group=short_code),alpha=0.5)+
  geom_text_repel(size=2.5,segment.color="black",alpha=1,segment.size = 0.2,#segment.size = 2,box.padding=0.1,alpha=1,
                  data=subset(dataplot,dataplot$snowyrlo==1)) + 
...
plot_limnoflush<-ggplot(dataplot, aes(x=flush_index_SWE_May, y=value, group="black", color=(park_code),label=short_code)) +
  annotation_logticks(sides="b",color="gray",size=.3)+
  geom_line(aes(group=short_code),alpha=0.5)+
  geom_text_repel(size=2,segment.color="black",alpha=1,segment.size = 0.2,box.padding=0.1,#alpha=1,
                  data=subset(dataplot,dataplot$snowyrlo==1)) + 
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
...
plot_chemsflush<-ggplot(dataplot, aes(x=flush_index_SWE_May, y=value, group="black", color=(park_code),label=short_code)) +
  annotation_logticks(sides="l",color="gray",size=.3)+
  annotation_logticks(sides="b",color="gray",size=.3)+
  geom_line(aes(group=short_code),alpha=0.5)+
  geom_text_repel(size=2,segment.color="black",alpha=1,segment.size = 0.2,box.padding=0.1,#,alpha=1,
                  data=subset(dataplot,dataplot$snowyrlo==1)) + 
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) + scale_y_log10() +
  xlab(dataplot$xaxis_label)+ylab("Log10 Value")+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  facet_wrap(~axis_label,ncol=2,scales="free_y") +
  theme(legend.position="none")+
