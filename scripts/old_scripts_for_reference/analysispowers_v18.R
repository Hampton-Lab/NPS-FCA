...
data<- data_forstats %>% filter(variable=="ProfTemp_top2m")
lme<-lme(na.action=na.omit,value ~ snowyr:SWE_May_snotel+snowyr:solar_jas+snowyr:Volume_m3-1, random = ~ 1|park_site/snowyr, method="ML",data=data) %>% as.data.frame() %>% filter(effect!="ran_pars") %>% as.data.frame()
data<- data_forstats %>% filter(variable %in% c("Chlorophyll","Total N","Total P","ProfTemp_top2m")) %>% 
  select(variable,park_site,snowyr,solar_jas,value) %>% unique() %>% as.data.frame() %>%
  spread(variable,value) %>% rename(TotalN="Total N",TotalP="Total P")
lme<-lme(na.action=na.omit,Chlorophyll ~ snowyr:TotalN+snowyr:TotalP+snowyr:solar_jas+snowyr:ProfTemp_top2m-1, random = ~ 1|park_site/snowyr, method="ML",data=data) %>% as.data.frame() %>% filter(effect!="ran_pars") %>% as.data.frame()

mi_glance<-glance(lme(na.action=na.omit,value ~ snowyr:vari-1, random = ~ 1|park_site/snowyr, method="ML",data = .)) %>% as.data.frame() %>% mutate(model=i+length(vars)) %>% select(variable,sigma,logLik,AIC,BIC,model)
