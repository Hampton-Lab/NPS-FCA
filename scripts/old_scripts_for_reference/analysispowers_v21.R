...
table_label_wunits_tbl <- tribble(
  ~variable, ~axis_label,
  "secchi_value_m",     "Secchi(m)",
  "ProfTemp_top2m", paste0("Watertemp(", intToUtf8(176), "C)"),
  "SWE_May", "SWE(cm)",
  "DO_top2m", "DO(mg/L)",
  "pH_top2m", "pH, top 2m",
  "SpCond_top2m", paste0("Condu(", intToUtf8(956), "S/cm), top 2m"),
  "Chlorophyll", paste0("Chl(", intToUtf8(956), "g/L)"),
  "Na", "Na(mg/L)",
  "Cl", "Cl(mg/L)",
  "Mg", "Mg(mg/L)",
  "Ca", "Ca(mg/L)",
  ...
  "Total N", "TDN(mg/L)", # IMPORTANT! - these were filtered samples 
  "Total P", "TDP(mg/L)", # IMPORTANT! - these were filtered samples   
  "ice_out_doy","Iceout day",
  "flush_index_SWE_May","Snowvol:lakevol"
)

table_label_nonits_tbl <- tribble(
  ~variable, ~axis_label,
  "secchi_value_m",     "Secchi(m)",
  "ProfTemp_top2m", paste0("Watertemp(", intToUtf8(176), "C)"),
  "SWE_May", "SWE(cm)",
...
#    mi<-lme(na.action=na.omit, value_model ~ vari1, random = ~vari1|park_site, method="ML",daterr) #%>% as.data.frame() %>% filter(effect!="ran_pars")# %>% as.data.frame() %>% mutate(model=paste("2_",i,sep=""))
#    mi<-summary(mi)$tTable %>% as.data.frame()
#    mi<-data.frame(varname,term=row.names(mi),mi,row.names=NULL)
#    daterr$y.hat <- predict(mi)
    
#    mi_glance<-glance(lme(na.action=na.omit, value_model ~ vari1, random = ~ 1|park_site, method="ML",data = daterr)) %>% as.data.frame() #%>% mutate(model=paste("2_",i,sep="")) %>% select(variable,sigma,logLik,AIC,BIC,model)
...
#  m_1covar<-m_1covar %>% select(-c(group,effect,BIC,deviance,statistic))#,-fixed))#,BIC,deviance,statistic))
#  m_1covar<-m_1covar %>% select(-c(BIC,deviance,statistic))#,-fixed))#,BIC,deviance,statistic))
  if(usenlme==0){
    m_1covar<-m_1covar %>% select(-c(nfix,BIC,deviance,statistic))#,-fixed))#,BIC,deviance,statistic))
  }
  if(usenlme==1){
    m_1covar<-m_1covar %>% select(-c(group,effect,BIC,deviance,statistic))#,-fixed))#,BIC,deviance,statistic))
  }
...
#    vari<-varname
    ...
    daterr<-daterr %>% select(value_model,vari1,vari2,park_site) %>% na.omit()
...
#  m_2covar<-m_2covar %>% select(-c(group,effect,BIC,deviance,statistic))#,-fixed))#,BIC,deviance,statistic))
#  m_2covar<-m_2covar %>% select(-c(BIC,deviance,statistic))#,-fixed))#,BIC,deviance,statistic))
  if(usenlme==0){
    m_2covar<-m_2covar %>% select(-c(nfix,BIC,deviance,statistic))#,-fixed))#,BIC,deviance,statistic))
  }
  if(usenlme==1){
    m_2covar<-m_2covar %>% select(-c(group,effect,BIC,deviance,statistic))#,-fixed))#,BIC,deviance,statistic))
  }
