...
if(doplots==1) {
  ...
  plotit<-ggplot(daterr_wallgrp, aes(x = fitted, y = value_model, col=park_code))+#,group=nothing)) +
...
#                mapping=aes(x = fitted, y = value_model),col="dark gray")+
  ...
  geom_line(data = cbind(daterr, y.hat = predict(mi)), aes(x = var1, y = y.hat),alpha=0.5) + 
...
#    theme(legend.position="none")+
  ...
  bestmodelfile1_bylake<-paste("../figures/analysispowers/best_",j,varname,bestmodel$ncovars,"varbylake_",
                        bestmodel_covars[j],"_",bestmodel$fixedpart,".png",sep="")
...
#    aes(color=park_site2)+
  ...
  bestmodelfile1_bypark<-paste("../figures/analysispowers/best_",j,varname,bestmodel$ncovars,"varbypark_",
                               bestmodel_covars[j],"_",bestmodel$fixedpart,".png",sep="")
...
#    daterr$var2<-daterr[,which(names(daterr) == bestmodel_covars[2])]
    ...
    plotit2<-ggplot(daterr, aes(x = var, y = value_model, col=park_site))+#,shape=snowyr)) +
      geom_line(data = cbind(daterr, y.hat = predict(mi)), aes(x = var, y = y.hat),alpha=0.5) + 
...
#    daterr$var3<-daterr[,which(names(daterr) == bestmodel_covars[3])]
    ...
    plotit3<-ggplot(daterr, aes(x = var, y = value_model, col=park_site))+#,shape=snowyr)) +
      geom_line(data = cbind(daterr, y.hat = predict(mi)), aes(x = var, y = y.hat),alpha=0.5) + 
      geom_point(alpha = 0.5,aes(x = var, y = value_model, col=park_site,shape=snowyr))+    
      #    theme(legend.position="none")+
      guides(shape=FALSE)+
  ...
  plotit2<-ggplot(daterr, aes(x = var2, y = value_model, col=park_site))+#,shape=snowyr)) +
    geom_line(data = cbind(daterr, y.hat = predict(mi)), aes(x = var2, y = y.hat),alpha=0.5) + 
...
#    theme(legend.position="none")+
  ...
  if(length(bestmodel_covars)>2){
    ...
    plotit3<-ggplot(daterr, aes(x = var3, y = value_model, col=park_site))+#,shape=snowyr)) +
      geom_line(data = cbind(daterr, y.hat = predict(mi)), aes(x = var3, y = y.hat),alpha=0.5) + 
