...
bigjoin<- bigjoin %>% rename(SWE_May_snodas=SWE_May, SWE_Apr_snodas=SWE_Apr,
...
# drop "Lake" from lake names to save table space
...
sitestablesimple_meansbypark<-sitestable %>% group_by(Park) %>% select(Elev_m,Elevwshed_m,
                                                                       SurfArea_ha,ShedArea_ha,ShedAreaLakeArea_ratio,
                                                                       Depthmax_m,Depthmean_m,Vol_m3xe6,#Shorelength_m,
                                                                       Aspect,SolarJAS_Whrperm2,SolarDec_Whrperm2,
                                                                       snowice,forest,barren,
...
sitestablesimple_means<-sitestable %>% mutate(Park="overall") %>% group_by(Park) %>% select(Elev_m,Elevwshed_m,
                                                                      SurfArea_ha,ShedArea_ha,ShedAreaLakeArea_ratio,
                                                                      Depthmax_m,Depthmean_m,Vol_m3xe6,#Shorelength_m,
                                                                      Aspect,SolarJAS_Whrperm2,SolarDec_Whrperm2,
                                                                      snowice,forest,barren,
...
#one$salmo_adult_ever[which(one$salmo_adult_ever==0)]<-"abs"
#one$salmo_adult_ever[which(one$salmo_adult_ever==1)]<-"pres"
...
#                                           count2=sum(count))
#three<-two %>% mutate(text=paste(salmo_adult_ever,"=",count,sep="")) %>% select(-salmo_adult_ever,-count)
#three<-two %>% mutate(text=count) %>% select(-salmo_adult_ever,-count)
salmocats_bypark<-two %>% mutate(text=count1)#mutate(text=paste(count1," of ",count2,sep=""))
#salmocats_bypark<-three %>% group_by(park_code) %>% summarise_each(funs(paste(., collapse = "/"))) %>% as.data.frame()
two<-one %>% mutate(park_code="overall") %>% group_by(park_code,salmo_adult_ever,.drop=FALSE) %>% dplyr::summarize(count=length(Lake)) %>% 
  dplyr::summarize(count1=sum(count*salmo_adult_ever))#,
#                   count2=sum(count))
#two<-one%>% mutate(park_code="overall",hydro=factor(salmo_adult_ever)) %>% group_by(park_code,salmo_adult_ever,.drop=FALSE) %>% dplyr::summarize(count=length(Lake))
#three<-two %>% mutate(text=paste(salmo_adult_ever,"=",count,sep="")) %>% select(-salmo_adult_ever,-count)
salmocats<-two %>% mutate(text=count1)#mutate(text=paste(count1," of ",count2,sep=""))
#salmocats<-three %>% summarise_each(funs(paste(., collapse = ", "))) %>% as.data.frame()
...
write.csv(categoricals,"../figures/analysispowers/categoricals.csv")
...
data_forstats$snowyrlo_snotel<-0;
data_forstats$snowyrlo_snotel[which(data_forstats$year==data_forstats$minYear_SWE_May_snotel)]<-1
#data_forstats$snowyrlo_snotel[which(data_forstats$SWE_May_snotel==data_forstats$min_SWE_May_snotel)]<-1
data_forstats$snowyrhi_snotel<-0;
data_forstats$snowyrhi_snotel[which(data_forstats$year==data_forstats$maxYear_SWE_May_snotel)]<-1
#data_forstats$snowyrhi_snotel[which(data_forstats$SWE_May_snotel==data_forstats$max_SWE_May_snotel)]<-1
data_forstats$snowyrmid_snotel<-0;
...
data_forstats$snowyrlo_snodas<-0;
data_forstats$snowyrlo_snodas[which(data_forstats$year==data_forstats$minYear_SWE_May_snodas)]<-1
#data_forstats$snowyrlo_snodas[which(data_forstats$SWE_May==data_forstats$min_SWE_May_snodas)]<-1
data_forstats$snowyrhi_snodas<-0;
data_forstats$snowyrhi_snodas[which(data_forstats$year==data_forstats$maxYear_SWE_May_snodas)]<-1
#data_forstats$snowyrhi_snodas[which(data_forstats$SWE_May_snodas==data_forstats$max_SWE_May_snodas)]<-1
data_forstats$snowyrmid_snodas<-0;
...
data_forstats$BlueLineInlet[which(data_forstats$BlueLineInlet=="F")]<-0
data_forstats$BlueLineInlet[which(data_forstats$BlueLineInlet=="T")]<-1
data_forstats$BlueLineOutlet[which(data_forstats$BlueLineOutlet=="F")]<-0
data_forstats$BlueLineOutlet[which(data_forstats$BlueLineOutlet=="T")]<-1
data_forstats$BlueLineInlet<-as.numeric(data_forstats$BlueLineInlet)
data_forstats$BlueLineOutlet<-as.numeric(data_forstats$BlueLineOutlet)
...
#data_forstats$snowyr<-factor(data_forstats$snowyr,levels=c("lo","mid","hi"))
...
meanval_SWEmidlohi<-data_forstats %>% group_by(variable,snowyrmid_snotel,snowyrlo_snotel,snowyrhi_snotel) %>%
...
#lme_detail<- lme(value ~ snowyr, random = ~ 1|park_site, method="ML",data=data_forstats_detail)
...
m_0covar_allvar_out<-m_0covar_allvar_out %>% mutate(b0=paste(val.int," ± ",se.int,sep=""),
                               Mlo=paste(val.lo," ± ",se.lo,sig.lo,sep=""),
                               Mhi=paste(val.hi," ± ",se.hi,sig.hi,sep="")) %>% 
  select(variable,b0,Mlo,Mhi)

write.csv(m_0covar_allvar_out,file="../figures/analysispowers/allvar_out.csv",fileEncoding = "UTF-16LE",row.names=FALSE,quote=FALSE)


#write.table(m_0covar_allvar_out,file=="allvar_out.csv",sep = "\t",row.names=FALSE)

# function to fit mixed models using 1 predictor
mod_1covar<-function(covars,data,groupvar,groupval=1){
  for (i in 1:length(covars)){
    ...
    mi$term<-vari #str_replace(mi$term,"vari",vari)
    if(i==1){m_1covar<-mi}
...
relate <-m_1covar %>% mutate_if(is.numeric, signif, digits=3) %>% select(-model) %>% arrange(variable,AICcdiff) %>% 
  filter(AICcdiff<=2)
#filter(AICcdiff<=2) %>% select(-effect,-group,-nobs,-nparam,-df,-BIC,-minAICc,-AIC) %>% rename(val=estimate,pval=p.value)
...
varorder <- tribble(
  ...
  "Depth_mean_m",     "2",
  "SWE_May_snotel",     "3",
  "Watershed_area_to_lake_area_ratio",     "4",
  "flush_index_SWE_May_snodas","5",
  "solar_jas",     "6",
  "northface",     "7",
  "forest",     "8",
  "barren",     "9")
widestats_out <- full_join(x = widestats_out, y = varorder, by = c("term"))
widestats_out<-widestats_out %>% arrange(order) %>% select(-order)
write.csv(widestats_out,"../figures/analysispowers/widestats.csv")
#############################
# function to fit mixed models using 2 predictors
mod_2covar<-function(covars,data,groupvar,groupval=1){
...
m_2covar0<-mod_2covar(covars=vars_2covar,data=data_forstats,groupval=1)
m_2covar<-m_2covar0
...
write.csv(m_2covar,"../figures/analysispowers/relate_2covar.csv",row.names=FALSE)
###########################
# do plots 
dataplot<-data_forstats %>% filter((snowyrlo_snotel==1 | snowyrhi_snotel==1), variable %in% c("ProfTemp_top2m"))
#dataplot<-data_forstats %>% filter((year==minYear_SWE_May_snotel | snowyrhi_snotel==1), variable %in% c("ProfTemp_top2m"))
...
dataplot<-data_forstats %>% filter((snowyrlo_snotel==1 | snowyrhi_snotel==1), variable %in% c("DO_top2m","Chlorophyll", #"Chlorophyll_log10", 
...
dataplot$value[which(dataplot$short_code=="TR" & dataplot$snowyrlo_snotel==1 & 
                       dataplot$variable=="Chlorophyll")]<-2.5 
dataplot$short_code[which(dataplot$short_code=="TR" &  
                            dataplot$variable=="Chlorophyll")]<-"TR*"
dataplot$value[which(dataplot$short_code=="MI" & dataplot$snowyrlo_snotel==1 & 
                       dataplot$variable=="secchi_value_m")]<-1.0 
dataplot$short_code[which(dataplot$short_code=="MI" &  
                            dataplot$variable=="secchi_value_m")]<-"MI*"
...
dataplot$value[which(dataplot$short_code=="LE" & dataplot$year==2011 & 
                       dataplot$variable=="SO4")]<-8.5 
dataplot$short_code[which(dataplot$short_code=="LE" &  
                            dataplot$variable=="SO4")]<-"LE*"
...
png(file="../figures/analysispowers/chemsflush.png",width=4.8,height=4,units="in",res=600)
plot_chems
dev.off()
############################### 
...
crosscorr<-subset(data_forstats,data_forstats$variable=="ProfTemp_top2m" & data_forstats$snowyrhi_snotel==1) %>% 
  ...
  Elevation_m,
  elevmean_wshed,
  Volume_m3,Depth_mean_m,
  Watershed_area_ha,Surface_area_ha,
  Watershed_area_to_lake_area_ratio,
  SWE_May_snotel,SWE_May_snodas,
  solar_dec,solar_jas,
  northface,
  flush_index_SWE_May_snotel,flush_index_SWE_May_snodas,
  forest,snowice,barren,                                                                                              
  BlueLineInlet,BlueLineOutlet,
  salmo_adult_ever) %>% 
  group_by(park_site) %>%
  summarise_all(list(~mean(.))) %>% 
  select(-park_site) %>% as.data.frame()
#  dplyr::summarize(meanSWE_snotel=mean(SWE_May_snotel,na.rm=TRUE),
#                   meanSWE_snodas=mean(SWE_May_snodas,na.rm=TRUE),
#                   flush_index_snotel=mean(flush_index_SWE_May_snotel,na.rm=TRUE),
#                   flush_index_snodas=mean(flush_index_SWE_May_snodas,na.rm=TRUE)) %>%
#                   select(-park_site) %>% as.data.frame() #-ice_out_doy,
#crosscorr2<-subset(data_forstats,data_forstats$variable=="ProfTemp_top2m" & data_forstats$year==data_forstats$minYear_SWE_May_snotel) %>% select(park_site,SWE_May_snotel,flush_index_SWE_May_snotel) %>% as.data.frame()
#crosscorr<-merge(crosscorr1,crosscorr2, by=c("park_site"),suffixes=c(".hi",".lo")) %>% select(Elevation_m,
#                                                                                              elevmean_wshed,
#                                                                                              Volume_m3,Depth_mean_m,
#                                                                                              Watershed_area_ha,Surface_area_ha,
#                                                                                              Watershed_area_to_lake_area_ratio,
