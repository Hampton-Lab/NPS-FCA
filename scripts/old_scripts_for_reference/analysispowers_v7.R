...
widestats_snowyrlo<-longstats_slopes_sig_trunc %>% filter(snowyr=="lo") %>%
  group_by(term,snowyr,dir) %>% dplyr::summarise(vars = paste(variable, collapse=", ")) %>% as.data.frame()
widestats_out<-merge(widestats_snowyrlo,widestats_snowyrhi,by="term",all=TRUE,suffixes=c("_lo","_hi"))
...
crosscorr1<-subset(data_forstats,data_forstats$variable=="ProfTemp_top2m" & data_forstats$year==2011) %>% select(-variable,-year,-short_code,-park_code,-snowyrlo,-snowyrhi,-snowyrmid,-value,-BlueLineInlet,-BlueLineOutlet,-ice_out_doy) %>% as.data.frame() #-ice_out_doy,
...
#crosscorr$Asp[which(crosscorr$northface_wshed==1)]<-1
#crosscorr$Asp[which(crosscorr$northface_wshed==0)]<-0
...
#iceout_SWE<-iceout_SWE %>% group_by(short_code) %>% mutate(SWE_rank = stat_ecdf(SWE_May)) %>% as.data.frame()
iceout_SWE<-iceout_SWE %>% group_by(short_code) %>% mutate(SWE_rank = dense_rank((SWE_May))/max(dense_rank((SWE_May)))) %>% as.data.frame()
#iceout_SWE<-iceout_SWE %>% mutate(SWE_rank = dense_rank((SWE_May))/max(dense_rank((SWE_May)))) %>% as.data.frame()

#iceout_SWE$year_label[-which(iceout_SWE$year_label %in% c("11","15"))]<-""
iceout_SWE_plot<-ggplot(iceout_SWE,aes(x=SWE_May,y=ice_out_doy,color=park_code,label=year_label))+
  xlab("May SWE")+ylab("Ice-out Day of Year")+
  geom_point(size=2)+
#  geom_text(size=3)+
  theme_bw()+
#  geom_text_repel(size=4,box.padding=0.05,color="black",segment.size = 0.4)+#segment.alpha=0)+
#  geom_text_repel(iceout_SWE %>% filter(year %in% c(2011,2015)), size=3,box.padding=0.05,color="black",segment.size = 0.4)
#  geom_line(aes(group=short_code),alpha=0.5)+
  geom_text_repel(size=3,box.padding=0.05,color="black",segment.size = 0.4)+#segment.alpha=0)+#segment.color="black")+#,segment.size = 0.2)+#,#segment.size = 2,box.padding=0.1,alpha=1,
...
iceout_SWE_plot<-ggplot(data=iceout_SWE,aes(y=SWE_rank,x=SWE_May,color=park_code,label=year_label))+
  geom_point()+
  ylab("Cumulative Distribution of May SWE")+xlab("May SWE (cm), or Ice-Out Day of Year")+
  geom_text_repel(size=3,box.padding=0.05,color="black",segment.size = 0.4)+#segment.alpha=0)+#segment.color="black")+#,segment.size = 0.2)+#,#segment.size = 2,box.padding=0.1,alpha=1,
  #                  data=subset(dataplot,dataplot$year==2015)) + 
  geom_point(data=iceout_SWE,aes(y=SWE_rank,x=ice_out_doy,color=park_code),shape=2)+
  theme_bw()+
...
iceout_SWE_plot<-ggplot(data=iceout_SWE,aes(x=SWE_May,color=park_code,label=year_label))+ stat_ecdf(geom="step")+
#  geom_point()+
  ylab("Empirical Distribution Function")+xlab("May SWE, or Ice-Out Day of Year")+
...
#  geom_text_repel(size=3,box.padding=0.05,color="black",segment.size = 0.4)+#segment.alpha=0)+#segment.color="black")+#,segment.size = 0.2)+#,#segment.size = 2,box.padding=0.1,alpha=1,
  #                  data=subset(dataplot,dataplot$year==2015)) + 
#  geom_point(data=iceout_SWE,aes(x=ice_out_doy,color=park_code),shape=2)+
  theme_bw()+
  #  theme(axis.title.y=element_blank(),
  #        axis.text.y=element_blank(),
  #        axis.ticks.y=element_blank())+
...
ggsave(filename = "../figures/analysispowers/iceout_SWE_plot.png",
       plot = iceout_SWE_plot, width = 10, height = 4.5, units = "in")
