...
uniqueparklakes<-bigjoin.stats %>% select(park_code,Lake) %>% unique() %>% arrange(park_code,Lake)
...
######################################
# sites table
sitestable0<-bigjoin.stats %>% select(park_code,short_code,Elevation_m,elevmean_wshed,Watershed_area_ha,Surface_area_ha,Volume_m3,Depth_max,Depth_mean_m,Shoreline_length_m,Watershed_area_to_lake_area_ratio,Aspect,solar_dec,solar_jas,snowice,forest,barren,hydro,
...
one<-bigjoin.stats %>% select(park_code,Lake,hydro) %>% mutate(hydro1=as.numeric(hydro==1),hydro2=as.numeric(hydro==2),hydro3=as.numeric(hydro==3),) %>% unique()
...
one0<-bigjoin.stats %>% mutate(andesite=as.numeric(andesite>50),basalt=as.numeric(basalt>50),biotitegneiss=as.numeric(biotitegneiss>50),
  ...
  unique()
one<-one0 %>% group_by(park_code) %>% select(Lake,park_code,andesite,basalt,biotitegneiss,granodiorite,graywacke,quartzmonzonite,quartzmonzodiorite,sandstone) %>% 
...
two<-one0 %>% mutate(park_code=as.character("overall")) %>% select(park_code,andesite,basalt,biotitegneiss,granodiorite,graywacke,quartzmonzonite,quartzmonzodiorite,sandstone) %>% 
  group_by(park_code) %>%
  dplyr::summarize(andesite=sum(andesite),
                   basalt=sum(basalt),
                   biotitegneiss=sum(biotitegneiss),
...
dataplot<-bigjoin.stats %>% select(park_code,short_code,park_site,year, SWE_May_snotel,days_since_iceout,ice_free_days,ice_out_doy) %>% unique() %>% as.data.frame() %>% na.omit()
...
data_forstats<- bigjoin.stats %>% 
  ...
  select(variable,var_abbrev1,var_abbrev2,axis_label,year,short_code,park_code,park_site,park_siteshort,Depth_mean_m,Volume_m3,Surface_area_ha,Watershed_area_ha,Watershed_area_to_lake_area_ratio,Elevation_m,elevmean_wshed,
         ...
         value,
         ice_out_doy,ice_in_doy,ice_free_days,SWE_May_snodas,SWE_May_snotel,flush_index_SWE_May_snodas,flush_index_SWE_May_snotel,BlueLineInlet,BlueLineOutlet,hydro,
         salmo_adult_ever,
...
data_forstats %>% select(park_code,park_site,year,ice_out_doy,SWE_May_snotel) %>% unique() %>%
...
#mi_glance<-do(by_variablei, glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
...
#mi_glance_iris<-do(group_by(iris,Species), glance(lme(na.action=na.omit,Petal.Width ~ Petal.Length+Sepal.Length, random = ~ 1|Sepal.Width, method="ML",data = .))) %>% as.data.frame() #%>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
#mi_glance_iris<-do(group_by(iris,Species), glance(lme(na.action=na.omit,Petal.Width ~ Petal.Length+Sepal.Length, random = ~ 1|as.factor(Sepal.Width), method="ML",data = .))) %>% as.data.frame() #%>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)

# Fine. One by one then :(
mi_glance1<-do(by_variablei %>% filter(variable=="ProfTemp_top2m"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance2<-do(by_variablei %>% filter(variable=="SO4"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance3<-do(by_variablei %>% filter(variable=="secchi_value_m"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance4<-do(by_variablei %>% filter(variable=="Na"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance5<-do(by_variablei %>% filter(variable=="Total P"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance6<-do(by_variablei %>% filter(variable=="Total N"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance7<-do(by_variablei %>% filter(variable=="Cl"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance8<-do(by_variablei %>% filter(variable=="K"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance9<-do(by_variablei %>% filter(variable=="Chlorophyll"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance10<-do(by_variablei %>% filter(variable=="SpCond_top2m"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance11<-do(by_variablei %>% filter(variable=="pH_top2m"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance12<-do(by_variablei %>% filter(variable=="Ca"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance13<-do(by_variablei %>% filter(variable=="DO_top2m"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
mi_glance14<-do(by_variablei %>% filter(variable=="Mg"), glance(lme(na.action=na.omit,value ~ snowyrlo_snotel+snowyrhi_snotel, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=1) %>% select(variable,sigma,logLik,AIC,BIC,model)
...
# dangit vctrs again!
# m_0covar_intonly_glance<-do(by_variablei, glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance1<-do(by_variablei %>% filter(variable=="ProfTemp_top2m"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance2<-do(by_variablei %>% filter(variable=="SO4"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance3<-do(by_variablei %>% filter(variable=="secchi_value_m"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance4<-do(by_variablei %>% filter(variable=="Na"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance5<-do(by_variablei %>% filter(variable=="Total P"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance6<-do(by_variablei %>% filter(variable=="Total N"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance7<-do(by_variablei %>% filter(variable=="Cl"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance8<-do(by_variablei %>% filter(variable=="K"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance9<-do(by_variablei %>% filter(variable=="Chlorophyll"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance10<-do(by_variablei %>% filter(variable=="SpCond_top2m"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance11<-do(by_variablei %>% filter(variable=="pH_top2m"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance12<-do(by_variablei %>% filter(variable=="Ca"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance13<-do(by_variablei %>% filter(variable=="DO_top2m"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)
m_0covar_intonly_glance14<-do(by_variablei %>% filter(variable=="Mg"), glance(lme(na.action=na.omit,value ~ 1, random = ~ 1|park_site, method="ML",data = .))) %>% as.data.frame() %>% mutate(model=0) %>% select(variable,sigma,logLik,AIC,BIC,model)

m_0covar_intonly_glance<-rbind(mi_glance1,mi_glance2,mi_glance3,mi_glance4,mi_glance5,mi_glance6,mi_glance7,mi_glance8,mi_glance9,mi_glance10,
                 mi_glance11,mi_glance12,mi_glance13,mi_glance14)
...
m_0covar_allvar_out<-m_0covar_allvar_out %>% arrange(order) %>% select(-order)
...
crosscorr_allyr<-data_forstats %>% select(park_site,variable,value,year) %>% group_by(park_site,variable,year) %>% dplyr::summarize(value=mean(value,na.rm=TRUE)) %>% spread(variable,value) %>% as.data.frame() %>% select(-park_site,-year) %>% na.omit()
...
mod_covar<-function(covars,data,varname,fixedpart,randompart,doplots,
                    valuecol,swecol,iceoutdcol){
  for (i in 1:length(covars[1,])){
    ...
    covarsi<-covars[,i];
    daterr<-daterr %>% select(value_formodel,swe_formodel,iceoutd_formodel,covarsi,park_site,park_code,year,snowyr) %>% na.omit()
...
onevar<-merge(
  models_df %>% filter(modname %in% models_1signif$modname) %>% select(varname,term,estimate,std.error,p.value,modname) %>%
  ...
  select(varname,term,coef,int,p.value_slope,p.value_int)#,std.error_int,p.value_int)
...
models_top_signif_df<-models_df %>% filter(term!= "(Intercept)" & p.value<=0.05) %>% select(varname,modname,modname2) %>% unique()
...
covartbl<-
  models_top_df %>% filter(modname %in% models_top_signif_df$modname) %>% select(varname,term,AICc,R,estimate,std.error,p.value,nparam,modname,modname2,modname3,var0,var1,var2,var3,varint1,varint2,varint3) %>%
  unique() %>%
  mutate(coef=paste(estimate," ± ",signif(std.error,1),sep="")) %>%
  mutate(modname2=str_replace(modname2,":","_")) %>%
  rename(p=p.value) %>%
  select(varname,term,AICc,R,coef,p,nparam,modname,modname2,modname3,var0,var1,var2,var3,varint1,varint2,varint3) %>%
...
covartbl<-covartbl %>% select(-term0)
covartbl$coef4[which(substr(covartbl$coef4,1,1)>=0)]<-"+"
covartbl$coef4[which(substr(covartbl$coef4,1,1)=="-")]<-"-"
covartbl$coef5[which(substr(covartbl$coef5,1,1)>=0)]<-"+"
covartbl$coef5[which(substr(covartbl$coef5,1,1)=="-")]<-"-"
covartbl$coef6[which(substr(covartbl$coef6,1,1)>=0)]<-"+"
covartbl$coef6[which(substr(covartbl$coef6,1,1)=="-")]<-"-"

covartbl$coef0<-make_stars(covartbl$coef0,covartbl$p0)
covartbl$coef1<-make_stars(covartbl$coef1,covartbl$p1)
covartbl$coef2<-make_stars(covartbl$coef2,covartbl$p2)
covartbl$coef3<-make_stars(covartbl$coef3,covartbl$p3)
covartbl$coef4<-make_stars(covartbl$coef4,covartbl$p4)
covartbl$coef5<-make_stars(covartbl$coef5,covartbl$p5)
covartbl$coef6<-make_stars(covartbl$coef6,covartbl$p6)
...
covartbl<-covartbl %>% select(-c(p0,p1,p2,p3,p4,p5,p6))
...
covartbl<-covartbl %>% mutate(interactions=paste(coef4,term4,coef5,term5,coef6,term6))
...
covartbl<-covartbl %>% select(var=varname,model=modname2,k=nparam,AICc,R,
                              coef0,term1,coef1,
                              term2,coef2,
                              term3,coef3,
                              interactions,
...
covartbl$R<-signif(covartbl$R,digits=2)
...
desired_order <- data.frame(var=c("Temp","DO","pH","Secchi","Chl","TDN","SO4","Cond","SWE","iceoutd","Ca","Cl",
                                  "K","Mg","Na"))
desired_order$order<-1:length(desired_order[,1])
covartbl<-merge(desired_order,covartbl,by="var") 
covartbl<-covartbl %>% arrange(order,k,(AICc)) %>% select(-order)
write.csv(covartbl,file=paste(fileout,"_covar_sig.csv",sep=""),fileEncoding = "UTF-16LE",row.names=FALSE,quote=FALSE)
