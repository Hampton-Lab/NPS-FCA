...
#  coord_flip()+
  geom_boxplot(aes(x = axis_label_nounits, y = value_z), alpha = 0.7)+
...
mi_glance<-rbind(mi_glance1,mi_glance2,mi_glance3,mi_glance4,mi_glance5,mi_glance6,mi_glance7,mi_glance8,mi_glance9,mi_glance10,
      ...
      mi_glance15,mi_glance16,mi_glance17)
...
make_stars_interact <- function(vector,term,pval) {
  pval<-replace_na(pval,99)
  for (i in 1:length(vector)){
    starsi = ""
    termi = ""
...
onevar<-merge(
  models_df %>% filter(modname %in% models_1signif$modname) %>% dplyr::select(varname,term,estimate,std.error,p.value,modname) %>%
...
covartbl$coef4x[which(substr(covartbl$coef4,1,1)>=0)]<-"+"
covartbl$coef4x[which(substr(covartbl$coef4,1,1)=="-")]<-"-"
covartbl$coef5x[which(substr(covartbl$coef5,1,1)>=0)]<-"+"
covartbl$coef5x[which(substr(covartbl$coef5,1,1)=="-")]<-"-"
covartbl$coef6x[which(substr(covartbl$coef6,1,1)>=0)]<-"+"
covartbl$coef6x[which(substr(covartbl$coef6,1,1)=="-")]<-"-"
...
covartbl$coef4xstar<-make_stars_interact(covartbl$coef4x,covartbl$term4,covartbl$p4)
covartbl$coef5xstar<-make_stars_interact(covartbl$coef5x,covartbl$term5,covartbl$p5)
covartbl$coef6xstar<-make_stars_interact(covartbl$coef6x,covartbl$term6,covartbl$p6)

covartbl[which(covartbl$p4>0.05),]$term4<-""
covartbl[which(covartbl$p4>0.05),]$coef4<-""
covartbl[which(covartbl$p4>0.05),]$coef4star<-""
covartbl[which(covartbl$p5>0.05),]$term5<-""
covartbl[which(covartbl$p5>0.05),]$coef5<-""
covartbl[which(covartbl$p5>0.05),]$coef5star<-""
covartbl[which(covartbl$p6>0.05),]$term6<-""
covartbl[which(covartbl$p6>0.05),]$coef6<-""
covartbl[which(covartbl$p6>0.05),]$coef6star<-""
covartbl[which(covartbl$p4>0.05),]$coef4xstar<-""
covartbl[which(covartbl$p5>0.05),]$coef5xstar<-""
covartbl[which(covartbl$p6>0.05),]$coef6xstar<-""
...
covartbl[which(covartbl$p4<=0.05),]$coef4star<-paste(covartbl[which(covartbl$p4<=0.05),]$coef4star," ")
...
covartbl[which(covartbl$p5<=0.05),]$coef5star<-paste(covartbl[which(covartbl$p5<=0.05),]$coef5star," ")
...
covartbl[which(covartbl$p6<=0.05),]$coef6star<-paste(covartbl[which(covartbl$p6<=0.05),]$coef6star," ")

covartbl[which(covartbl$p4<=0.05),]$coef4x<-paste(covartbl[which(covartbl$p4<=0.05),]$coef4x," ")
covartbl[which(covartbl$p5<=0.05),]$coef5x<-paste(covartbl[which(covartbl$p5<=0.05),]$coef5x," ")
covartbl[which(covartbl$p6<=0.05),]$coef6x<-paste(covartbl[which(covartbl$p6<=0.05),]$coef6x," ")
...
covartbl$coef4x<-gsub("NA","",covartbl$coef4x)
covartbl$coef5x<-gsub("NA","",covartbl$coef5x)
covartbl$coef6x<-gsub("NA","",covartbl$coef6x)
covartbl$coef4star<-gsub("NA","",covartbl$coef4star)
covartbl$coef5star<-gsub("NA","",covartbl$coef5star)
covartbl$coef6star<-gsub("NA","",covartbl$coef6star)
covartbl$coef4xstar<-gsub("NA","",covartbl$coef4xstar)
covartbl$coef5xstar<-gsub("NA","",covartbl$coef5xstar)
covartbl$coef6xstar<-gsub("NA","",covartbl$coef6xstar)


#paste(substr(covartbl$coef4,1,1),covartbl$term4,paste(rep("*",str_count(covartbl$coef4,"\\*")),collapse=""))

#covartbl %>% mutate(interactions=
#    paste(substr(coef4,1,1),term4,paste(rep("*",str_count(coef4,"\\*")),collapse=""))
#    )

#  paste(substr(covartbl$coef4,1,1),covartbl$term4,paste(rep("*",str_count(covartbl$coef4[1],"\\*")),collapse=""))


#covartbl %>% mutate(interactions=paste(substr(coef4,1,1),term4,rep("*",str_count(coef4,"\\*")),
#  coef5,term5,coef6,term6))

covartbl<-covartbl %>% mutate(interactions=paste(coef4xstar,coef5xstar,coef6xstar))
...
covartbl$interactions<-gsub("  "," ",covartbl$interactions)
covartbl$interactions<-gsub("  "," ",covartbl$interactions)
# next line omits stars from interactions column
#covartbl$interactions<-gsub("\\*","",covartbl$interactions)
...
covartbl<-covartbl %>% dplyr::select(var=varname,model=modname2,k=nparam,AICc,R2m,R2c,
                              ...
                              term3,coef3,
                              interactions,
                              term4,coef4,term5,coef5,term6,coef6,
                              coef0star,coef1star,coef2star,
                              coef3star,coef4star,coef5star,coef6star,
                              -modname)
...
covartbl$term1supp<-paste(covartbl$term1,covartbl$coef1star)
covartbl$term2supp<-paste(covartbl$term2,covartbl$coef2star)
covartbl$term3supp<-paste(covartbl$term3,covartbl$coef3star)
covartbl$term4supp<-paste(covartbl$term4,covartbl$coef4star)
covartbl$term5supp<-paste(covartbl$term5,covartbl$coef5star)
covartbl$term6supp<-paste(covartbl$term6,covartbl$coef6star)

covartbl<-covartbl %>% mutate_all(funs(str_replace_all(., "- NA", "")))
covartbl<-covartbl %>% mutate_all(funs(str_replace_all(., "NA", "")))

desired_order$order<-1:length(desired_order[,1])
covartbl<-merge(desired_order,covartbl,by="var") 
covartbl_main<-covartbl %>% arrange(order,k,(AICc)) %>% 
  dplyr::select(var,model,k,AICc,R2m,b0=coef0,
    term1,m1=coef1,
    term2,m2=coef2,
    term3,m3=coef3,
    interactions)
...
#covartbl_nostars<-gsub("*","",covartbl)
#covartbl_nostars<-covartbl %>% mutate_all(funs(str_replace_all(., "\\*", "")))
#covartbl_nostars<-covartbl_nostars %>% mutate_all(funs(str_replace_all(., "\\. ", "")))
#write.csv(covartbl_nostars,file=paste(fileout,"_covar_sig_nostars.csv",sep=""),fileEncoding = "UTF-16LE",row.names=FALSE,quote=FALSE)
covartbl_supp<-covartbl %>% arrange(order,k,(AICc)) %>% 
...
which(covartbl_supp$int2==" " & covartbl_supp$int3!=" ")
