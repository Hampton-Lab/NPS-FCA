...
data_forstats<- bigjoin.stats %>% 
  ...
  select(variable,axis_label,year,short_code,park_code,park_site,park_siteshort,Depth_mean_m,Volume_m3,Surface_area_ha,Watershed_area_ha,Watershed_area_to_lake_area_ratio,Elevation_m,elevmean_wshed,#Asp,
         ...
         andesite,basalt,biotitegneiss,granodiorite,graywacke,quartzmonzodiorite,quartzmonzonite,sandstone,tholeiite,
         dep_names) %>%         
...
###########
# New model scetion started 15 May 2020
#data$NP<-(data$TotalN/15)/(data$TotalP/31+0.0005)
# note, N:P ratios meaningless due to P being low/below detection

#temperature model
data_formodel<- data_forstats %>% filter(variable=="ProfTemp_top2m")
lme<-lme(na.action=na.omit,value_z ~ snowyr:SWE_May_snotel+snowyr:solar_jas+snowyr:Volume_m3-1, random = ~ 1|park_site/snowyr, method="ML",data=data_formodel)

data_formodel<- data_forstats %>% filter(variable %in% c("Chlorophyll","secchi_value_m","Total N","Total P","ProfTemp_top2m")) %>% 
  select(variable,park_site,snowyr,solar_jas,forest,N_dep_1985_2015,value_z) %>% unique() %>% 
  tidyr::pivot_wider(names_from=variable,values_from=value_z,values_fn = list(value_z=mean)) %>% rename(TotalN="Total N",TotalP="Total P") %>% as.data.frame()

#secchi models
lme<-lme(na.action=na.omit,secchi_value_m ~ snowyr:Chlorophyll-1, random = ~ 1|park_site/snowyr, method="ML",data=data) 

#chlorophyll models
lme<-lme(na.action=na.omit,Chlorophyll ~ snowyr:TotalN+snowyr:TotalP+snowyr:solar_jas+snowyr:ProfTemp_top2m-1, random = ~ 1|park_site/snowyr, method="ML",data=data_formodel) 
lme<-lme(na.action=na.omit,Chlorophyll ~ snowyr:TotalN+snowyr:solar_jas+snowyr:ProfTemp_top2m-1, random = ~ 1|park_site/snowyr, method="ML",data=data_formodel) 
lme<-lme(na.action=na.omit,Chlorophyll ~ snowyr:TotalP+snowyr:solar_jas+snowyr:ProfTemp_top2m-1, random = ~ 1|park_site/snowyr, method="ML",data=data_formodel) 
lme<-lme(na.action=na.omit,Chlorophyll ~ snowyr:TotalN+snowyr:TotalP-1, random = ~ 1|park_site/snowyr, method="ML",data=data_formodel) 

# nitrogen model
lme<-lme(na.action=na.omit,TotalN ~ snowyr:forest+snowyr:N_dep_1985_2015-1, random = ~ 1|park_site/snowyr, method="ML",data=data_formodel) 

# phosphorus model
lme<-lme(na.action=na.omit,TotalP ~ snowyr:forest-1, random = ~ 1|park_site/snowyr, method="ML",data=data_formodel) 

# sulfate model
data_formodel<- data_forstats %>% filter(variable=="SO4")
lme<-lme(na.action=na.omit,value_z ~ snowyr:SO4_dep-1, random = ~ 1|park_site/snowyr, method="ML",data=data_formodel) 

# pH
data_formodel<- data_forstats %>% filter(variable=="pH_top2m")
lme<-lme(na.action=na.omit,value_z ~ snowyr:forest-1, random = ~ 1|park_site/snowyr, method="ML",data=data_formodel) 

#cations/ations
data_formodel<- data_forstats %>% filter(variable %in% c("Ca","Mg","K","Na","Cl")) %>% 
  select(variable,park_site,snowyr,solar_jas,forest,
         andesite,basalt,biotitegneiss,granodiorite,graywacke,quartzmonzodiorite,quartzmonzonite,sandstone,tholeiite,
         value_z) %>% unique() #%>% 
#  tidyr::pivot_wider(names_from=variable,values_from=value_z,values_fn = list(value_z=mean))  %>% as.data.frame()
#lme<-lme(na.action=na.omit,Ca ~ snowyr:andesite+snowyr:basalt+snowyr:biotitegneiss+snowyr:granodiorite+snowyr:graywacke+snowyr:quartzmonzodiorite+snowyr:quartzmonzonite+snowyr:sandstone+snowyr:tholeiite-1, random = ~ 1|park_site/snowyr, method="ML",data=data) 
#lme<-lme(na.action=na.omit,Ca ~ snowyr:andesite+snowyr:basalt+snowyr:biotitegneiss+snowyr:granodiorite+snowyr:graywacke+snowyr:quartzmonzodiorite+snowyr:quartzmonzonite+snowyr:sandstone+snowyr:tholeiite-1, random = ~ 1|park_site/snowyr, method="ML",data=data) 
by_variablei <- group_by(data_formodel, variable)
mi<-by_variablei %>% do(tidy(lme(na.action=na.omit,value_z ~ snowyr:andesite+snowyr:basalt+snowyr:biotitegneiss+snowyr:granodiorite+snowyr:graywacke+snowyr:quartzmonzodiorite+snowyr:quartzmonzonite+snowyr:sandstone+snowyr:tholeiite-1, random = ~ 1|park_site/snowyr, method="ML",data=.))) %>% as.data.frame() %>% filter(effect!="ran_pars") %>% as.data.frame()
mi_glance<-do(by_variablei, glance(lme(na.action=na.omit,value_z ~ snowyr:andesite+snowyr:basalt+snowyr:biotitegneiss+snowyr:granodiorite+snowyr:graywacke+snowyr:quartzmonzodiorite+snowyr:quartzmonzonite+snowyr:sandstone+snowyr:tholeiite-1, random = ~ 1|park_site/snowyr, method="ML",data = .))) %>% as.data.frame() %>% select(variable,sigma,logLik,AIC,BIC)


#mi_glance<-glance(lme(na.action=na.omit,Chlorophyll ~ snowyr:TotalN+snowyr:TotalP+snowyr:solar_jas+snowyr:ProfTemp_top2m-1, random = ~ 1|park_site/snowyr, method="ML",data = data)) %>% as.data.frame()
