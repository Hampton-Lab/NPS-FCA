...
mod_3covar<-function(covars,data,varname){
  for (i in 1:length(covars[1,])){
    var1i<-covars[,i][1];var2i<-covars[,i][2];var3i<-covars[,i][3]
    daterr<-data
    daterr$vari1<-daterr[,which(names(daterr)==var1i[1])]
    daterr$vari2<-daterr[,which(names(daterr)==var2i[1])]
    daterr$vari3<-daterr[,which(names(daterr)==var3i[1])]
    print(i)
    if(identical(daterr$vari1,daterr$vari2)==TRUE){
      print("perfectly correlated vars...")
      print(c(var1i,var2i))
      print("on to the next set...")
      next()}
    mi<-daterr %>% do(tidy(lme(na.action=na.omit, value_model ~ vari1*vari2*vari3, random = ~ 1|park_site, method="ML",data=.))) %>% as.data.frame() %>% filter(effect!="ran_pars")# %>% as.data.frame() %>% mutate(model=paste("2_",i,sep=""))
    ...
    mi$term[which(mi$term=="vari1:vari2")]<-paste(var1i,":",var2i,sep="")
    mi$term[which(mi$term=="vari2:vari1")]<-paste(var2i,":",var1i,sep="")
    mi$term[which(mi$term=="vari2:vari3")]<-paste(var2i,":",var3i,sep="")
    mi$term[which(mi$term=="vari1:vari3")]<-paste(var1i,":",var3i,sep="")
    mi$term[which(mi$term=="vari1:vari2:vari3")]<-paste(var1i,":",var2i,":",var3i,sep="")
    mi$term[which(mi$term=="vari1")]<-var1i
    mi$term[which(mi$term=="vari2")]<-var2i
    mi$term[which(mi$term=="vari3")]<-var3i
    if(i==1){m_3covar<-mi}
    if(i>1){m_3covar<-rbind(m_3covar,mi)}
  }
  m_3covar$nparam<-5
  m_3covar$nobs<-170+21+21
  m_3covar<-m_3covar %>% mutate(AICc=(-2*logLik)+(2*nparam)+(2*nparam*(nparam+1)/(nobs-nparam-1)))
  m_3covar<-m_3covar %>% select(-c(group,effect,BIC,deviance,statistic))#,-fixed))#,BIC,deviance,statistic))
  return(m_3covar)
}


file_models_zscore<-"../figures/analysispowers/modelouts_zscore.txt"
file_models_nozscore<-"../figures/analysispowers/modelouts_nozscore.txt"
...
dataplot$fitted<-as.numeric(lm$fitted.values)
dataplot$residuals_z<-as.numeric(lm$residuals)
...
dataplot$fitted<-as.numeric(lm$fitted.values)
dataplot$residuals<-as.numeric(lm$residuals)
