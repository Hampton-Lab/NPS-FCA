...
#### !! this rebeta changes the model chosen? why?? !!!  #####
    mi_order<-mi
    rbetai<-r2beta(mi_order) %>% as.data.frame()
...
#    if(length(covarsi)>1 & class(mi.relaimpo)!="try-error"){
    if(length(covarsi)>1 & class(mi0)=="lme"){
#    mi<-mi[match(mi$term,termorder),] # old way before rbeta
    mi<-mi[match(mi$term,c("(Intercept)",termorder)),] # old way before rbeta
    mi$order<-1:length(mi[,1])
    mi$order[which(mi$term=="(Intercept)")]<-1+length(mi[,1])
    if(length(grep(":",mi$term))>0){
    mi$order[grep(":",mi$term)]<-1+length(mi[,1])
    }
    mi<-mi %>% arrange(order)
    mi<-mi %>% select(-order)
#    mi$termorder<-0
#    mi$termorder[which(mi$term %in% c(termorder))]
#    mi<-
 #     mi[which(mi$term %in% c("(Intercept)",termorder)),]
    }
