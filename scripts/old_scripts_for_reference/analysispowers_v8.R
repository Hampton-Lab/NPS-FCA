...
data_forstats %>% filter(variable=="ProfTemp_top2m",value>-99) %>% 
  select(year,short_code,park_code,park_site,park_siteshort,value,SWE_May) %>%
#  mutate(hasproftop2m=value>-99) %>%
  group_by(park_site,short_code) %>%
  dplyr::summarize(min=min(SWE_May),minYear=year[which(SWE_May == min(SWE_May))][1]) %>% as.data.frame()


data_forstats %>% filter(variable=="ProfTemp_top2m") %>% 
  select(year,short_code,park_code,park_site,park_siteshort,value) %>%
  mutate(hasproftop2m=value>-99) %>%
  group_by(park_site,short_code,hasproftop2m) %>%
  dplyr::summarize(min=min(value),minYear = year[which(value == min(value))]) %>% as.data.frame()

left_join(data_forstats %>% filter(variable=="ProfTemp_top2m") %>% 
  select(year,short_code,park_code,park_site,park_siteshort,value) %>%
  mutate(hasproftop2m=value>-99),
  data_forstats %>% filter(variable=="SWE_May"),
  by=c("year","park_site"))
...
#iceout_SWE<-subset(iceout_SWE,iceout_SWE$short_code %in% c("15","AL","BL","PA","BO","ER","LE","SI","CR","HE","HO","LC"))
...
#  geom_text_repel(size=3,box.padding=0.15,color="black",segment.size = 0.4)+#segment.alpha=0)+#segment.color="black")+#,segment.size = 0.2)+#,#segment.size = 2,box.padding=0.1,alpha=1,
...
#  geom_text(data=iceout_SWE,aes(y=SWE_rank,x=ice_out_doy))+
  theme_bw()+
