...
maxyears<-data_forstats %>% filter(variable=="ProfTemp_top2m",value>-99) %>% 
  select(year,short_code,park_code,park_site,park_siteshort,value,SWE_May) %>%
  group_by(park_site,short_code) %>%
  dplyr::summarize(max=max(SWE_May),maxYear=year[which(SWE_May == max(SWE_May))][1]) %>% as.data.frame()


data_forstats<-merge(data_forstats,minyears,by=c("park_site"))
data_forstats$snowyrlo<-0
data_forstats$snowyrlo[which(data_forstats$year==data_forstats$minYear)]<-1
