...
bp_varorder<-c("Watertemp","DO","pH","Secchi","Chl","TDN","TDP","SO4","Cond","Ca","Cl","K","Mg","Na","clad","rot","cope")
...
#  coord_flip()+
  geom_boxplot(aes(x = axis_label, y = value_z), alpha = 0.7)+
...
#              aes(x = axis_label, y = value_z, color = snowyr)) + 
  scale_x_discrete(limits = rev(levels(factor(data_forstats$axis_label))))+
  xlab("")+
  ylab("z scored value")+
  coord_flip()+
...
crosscorr_hiy<-subset(data_forstats,data_forstats$snowyrhi_snotel==1) %>% dplyr::select(park_site,variable,value) %>% spread(variable,value) %>% dplyr::select(-park_site)
crosscorr_loy<-subset(data_forstats,data_forstats$year==data_forstats$minYear_SWE_May_snotel) %>% dplyr::select(park_site,variable,value) %>% spread(variable,value) %>% dplyr::select(-park_site)
crosscorr_allyr<-data_forstats %>% group_by(park_site,variable) %>% dplyr::summarize(value=mean(value,na.rm=TRUE)) %>% spread(variable,value) %>% as.data.frame() %>% dplyr::select(-park_site)
crosscorr_allyr<-data_forstats %>% dplyr::select(park_site,variable,value,year) %>% group_by(park_site,variable,year) %>% dplyr::summarize(value=mean(value,na.rm=TRUE)) %>% spread(variable,value) %>% as.data.frame() %>% dplyr::select(-park_site,-year) %>% na.omit()
...
#    Cand.models <- list( )
#    Cand.models[[1]] <- lme(na.action=na.omit, modelformula, random = randompart, method="ML",data=daterr) #%>% as.data.frame() #%>% filter(effect!="ran_pars")# %>% as.data.frame() %>% mutate(model=paste("2_",i,sep=""))
#    Cand.models[[2]] <- lme(na.action=na.omit, formula(value_formodel ~ (ice_out_doy + SWE_May_snotel + solar_jas)), random = randompart, method="ML",data=daterr) #%>% as.data.frame() #%>% filter(effect!="ran_pars")# %>% as.data.frame() %>% mutate(model=paste("2_",i,sep=""))
...
#    print(c("termorder",termorder))
#    print(c("covars_ordered",covars_ordered))
...
main_varorder<-c("Watertemp","DO","pH","Secchi","Chl","TDN","TDP","SO4","Cond","log10Clad","log10Rot","log10Cope","SWE","ice_out_doy","Clad","Rot","Cope")
...
covartbl<-
  models_top_df %>% filter(modname %in% models_top_signif_df$modname) %>% dplyr::select(varname,term,AICc,R,estimate,std.error,p.value,nparam,modname,modname2,modname3,var0,var1,var2,var3,varint1,varint2,varint3) %>%
  unique() %>%
...
covartbl<-covartbl %>% dplyr::select(var=varname,model=modname2,k=nparam,AICc,R,
                              coef0,term1,coef1,
                              term2,coef2,
                              term3,coef3,
                              interactions,
...
covartbl<-covartbl %>% arrange(order,k,(AICc)) %>% dplyr::select(-order)
