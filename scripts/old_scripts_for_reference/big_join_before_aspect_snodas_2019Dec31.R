...
# Veg cover data
veg_noca <- read_excel(path = "../data/MOLA_Watershed_Veg.xlsx",
                       sheet = "NOCA mola")
veg_mora <- read_excel(path = "../data/MOLA_Watershed_Veg.xlsx",
                       sheet = "MORA mola")
veg_olym <- read_excel(path = "../data/MOLA_Watershed_Veg.xlsx",
                       sheet = "OLYM mola")
...
water_prof <- clean_names(water_prof) %>%
  mutate(event_month = month(start_date),
         start_date = as.Date(start_date, format = "%Y-%m-%d"))
water_prof$parameter[which(water_prof$parameter=="DO_sat")]<-"DO_sat_incomplete"
water_prof<-merge(physio %>% as.data.frame(),water_prof,by.x="site_code",by.y="site_code")
#unique(water_prof2$Lake)

water_prof_sol<-water_prof %>% filter(parameter=="Temperature")
water_prof_sol$parameter<-"DOsol"
water_prof_sol$parameter_value<-
  o2.at.sat.base(water_prof_sol$parameter_value, altitude = water_prof_sol$Elevation_m, salinity = 0,
                 model = "garcia-benson")
water_prof<-rbind(water_prof,water_prof_sol)
water_prof_DO0<-water_prof %>% filter(parameter %in% c("DO"))  
water_prof_DOsol0<-water_prof %>% filter(parameter %in% c("DOsol"))  
bynames<-names(water_prof_DO0)[-which(names(water_prof_DO0) %in% c("parameter_value","parameter"))]
water_prof_DOsat<-merge(water_prof_DO0,water_prof_DOsol0,by=bynames,suffixes=c("",".sol"))
water_prof_DOsat$parameter_value<-100*water_prof_DOsat$parameter_value/water_prof_DOsat$parameter_value.sol
water_prof_DOsat<-water_prof_DOsat %>% select(-parameter_value.sol,-parameter.sol)
...
solar<-solar %>% select(lake,solar_jas)
solar$lake[which(solar$lake=="LP19")]<-"Lake LP19"
solar$lake[which(solar$lake=="LH15")]<-"Lake LH15"
solar$lake[which(solar$lake=="Lake LaCrosse")]<-"Lake La Crosse"

physio<-merge(physio,solar,by.x="Lake",by.y="lake")
...
sort(unique(park_veg$MAP_CLASS[grep("forest|woodland",park_veg$MAP_CLASS,ignore.case=TRUE)]))
sort(unique(park_veg$MAP_CLASS[grep("shrub",park_veg$MAP_CLASS,ignore.case=TRUE)]))
sort(unique(park_veg$MAP_CLASS[grep("meadow",park_veg$MAP_CLASS,ignore.case=TRUE)]))
sort(unique(park_veg$MAP_CLASS[grep("barren",park_veg$MAP_CLASS,ignore.case=TRUE)]))
...
veg_key<-rbind(veg_noca, veg_mora, veg_olym) %>%
         group_by(MAP_CLASS) %>%
         dplyr::summarize(sum_group = sum(Shape_Area)) %>% arrange(-sum_group) %>% as.data.frame()
...
park_veg_trim$forest<- rowSums(park_veg_trim %>% select(veg_M01Y,veg_M06,veg_M07D,
                                                        veg_M07W,veg_M17E,veg_M17M,
                                                        veg_M24,veg_M33,veg_M43M,
                                                        veg_M46A,veg_M46B,veg_M46D,
                                                        veg_M46S,veg_M47,veg_M50))
park_veg_trim$shrub<- rowSums(park_veg_trim %>% select(veg_M18,veg_M21,veg_M50,
                                                        veg_M74A,veg_M74S,veg_M85))
park_veg_trim$meadow<- rowSums(park_veg_trim %>% select(veg_M52,veg_M58,veg_M61,
                                                       veg_M62,veg_M67E,veg_M67W,
                                                       veg_M86))
park_veg_trim$barren<- rowSums(park_veg_trim %>% select(veg_M90,veg_M91,veg_M92,
                                                        veg_M93))
park_veg_trim$snowice<- rowSums(park_veg_trim %>% select(veg_M97))

park_veg_trim<- park_veg_trim %>% select(site_code,park_code,forest,shrub,meadow,barren,snowice)
...
fish_salmo_adult_ever <- fish_species_full %>%
  mutate(salmo = fish_species_full$true_taxon %in% c("RBT","CCT","BRK","WCT")) %>%
  group_by(park_code, site_code,
           variable = "salmo_adult_ever") %>%
  dplyr::summarize(value = as.numeric(max(true_count) > 0)) %>%
  arrange(site_code, variable) %>%
...
SWE_May_format <-  unique(filter(lakesnow_format,
                                 variable %in% "SWE_May") %>%
                            select(site_code, park_code, event_year,
                                   SWE_May = value))
