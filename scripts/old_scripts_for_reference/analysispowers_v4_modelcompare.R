...
m_single_minAIC<-m_single %>% group_by(variable) %>% 
  dplyr::summarize(minAIC=min(AIC))
m_combo_minAIC<-m_combo %>% group_by(variable) %>% 
  dplyr::summarize(minAIC=min(AIC))
m_single<-merge(m_single,m_single_minAIC,by.x=c("variable"),by.y=c("variable"))
m_single$AICdiff<-m_single$AIC-m_single$minAIC
m_combo<-merge(m_combo,m_combo_minAIC,by.x=c("variable"),by.y=c("variable"))
m_combo$AICdiff<-m_combo$AIC-m_combo$minAIC

stats<-rbind(m_single,m_combo)
stats<-stats %>% arrange(variable,AIC)
m_single_topmodels<-m_single %>% filter(AICdiff<=2) %>% arrange(variable,AIC,term) %>% as.data.frame()
