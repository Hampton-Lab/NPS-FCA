...
vars<-c("snowyr","Depth_mean_m","Volume_m3","Watershed_area_ha","Elevation_m",
        "SWE_May",#"SWE_Apr",
...
vars_1covar<-vars[-which(vars %in% c("snowyr"))] 
vars_2covar<-combn(vars[-which(vars %in% c("yr","park_code","snowyr","SWE_May","flush_index_SWE_May"))],2)
...
two<-one+
  geom_point(e_detail %>% filter(variable=="fit"),mapping=one$mapping,size=3,color="gray")+
  geom_line(e_detail  %>% filter(variable %in% c("lower","upper")),mapping=one$mapping,size=1,color="gray")
detail_wfit_sites<-two

one<-ggplot(data_forstats_detail,aes(y=value,x=snowyear,color=park_code,label=yearlab))+
  geom_text_repel(size=2,segment.color=NA,box.padding=0.1)+#size=2.5,segment.color="black",alpha=1,segment.size = 0.2) + 
  ylab(data_forstats_detail$axis_label[1])+
  xlab("Snow year")+
  theme_bw()+theme(legend.position="none")
