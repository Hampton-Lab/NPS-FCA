...
#bigjoin.stats[which(bigjoin.stats$variable=="DO_top2m" & bigjoin.stats$value<6.5),]
#bigjoin.stats[which(bigjoin.stats$Lake=="Lower East Lake" & bigjoin.stats$year==2016),]
...
#data_forstats<- data_forstats %>% filter(park_code!="OLYM")
#data_forstats<- data_forstats %>% filter(park_code!="OLYM")
...
#models_df_swe_iceout_doy<-models_df_swe_iceout_doy[which(names(models_df_swe_iceout_doy) %in% names)]
...
#covartbl_main$interactions[grep("- -",covartbl_main$interactions)]<-""
#str_replace(covartbl_main$interactions," - ","")
#str_replace(covartbl_main$interactions,"- ","")
#mutate(modname2=str_replace(modname2,":","_")) %>%

write.csv(covartbl_main,file=paste(fileout,"_covar_main.csv",sep=""),row.names=FALSE,quote=FALSE)
#write.csv(covartbl_main,file=paste(fileout,"_covar_main.csv",sep=""),fileEncoding = "UTF-16LE",row.names=FALSE,quote=FALSE)
#write.csv(covartbl_main,file=paste(fileout,"_covar_main.csv",sep=""),fileEncoding = "UTF-8",row.names=FALSE,quote=FALSE)
...
models_df0<-models_df
#which_interact<-grep("\\^",models_df$modname)
#which_interact<-grep("\\*",models_df0$modname)
...
#  geom_line(aes(x=a,y=preds,group=park_site),alpha=0.25)+
  geom_line(aes(x=a,y=preds),se=FALSE,color="gray")+#,group=park_site))+
#  geom_smooth(aes(x=a,y=preds),se=FALSE,color="gray")+#,group=park_site))+
  geom_point(size = ptsize,aes(x = a, y = value, color=park_code)) + # change size and colour
  scale_color_viridis_d(end = 0.85,na.value="gray")+
  theme_bw()+
  theme(legend.position="none",
...
#  geom_line(aes(x=b,y=preds),se=FALSE,color="gray")+#,group=park_site))+
  #  geom_smooth(aes(x=a,y=preds),se=FALSE,color="gray")+#,group=park_site))+
